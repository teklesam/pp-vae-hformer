...
% ── Output thumbnails ────────────────────────────────────────────────
% Centered at x=4.7 (0.9cm wide → west=4.25), so the arrow from headBr(x=3.8)
% goes up/down then turns RIGHT to arrive at each image from outside.
\node[ovImg] (outmu) at (4.7,10.2)
  {\includegraphics[width=0.9cm,height=0.9cm,keepaspectratio]%
    {figures/arch/arch_output_mu.png}};
\node[ovDim, above=2pt of outmu] {$\hat{\mu}_y$};

\node[ovImg] (outsig) at (4.7,9.0)
  {\includegraphics[width=0.9cm,height=0.9cm,keepaspectratio]%
...
% head → outputs: fork at headBr, then right-turn into each image
\coordinate (headBr) at ($(head.east)+(0.3,0)$);
