...
\begin{titlepage}
  ...
  {\large \today}\\[0.5cm]
  {\large Word Count: $\approx$24{,}270}
...
The total word count of this dissertation is approximately 24{,}270 words, which is within the limit stipulated by the MPhil Population Health Sciences regulations.
...
\begin{enumerate}
  ...
  \item \textbf{Pathology Preservation Validation:} To train the proposed model on the
        ...
        to seven denoising architectures retrained from scratch under identical Poisson-Gaussian noise conditions: five established baselines (DnCNN, IRCNN, FFDNet, DRUNet, SwinIR) and two current state-of-the-art extended comparisons (NAFNet \cite{chen_nafnet_2022} and SCUNet \cite{zhang_scunet_2023}).
...
This composite objective is evaluated through a 16-arm principal ablation study (Arms A–P) that systematically varies loss combinations, KL divergence annealing schedules, and architectural components. The core sequence — Arm~A (L2 baseline, no probabilistic bottleneck), Arms~B–D (adding NLL, then SSIM, then FFL sequentially), and Arm~E (full PP-VAE-Hformer with VAE bottleneck and cyclical KL) — isolates the contribution of each design decision. The remaining 11 arms probe three alternative KL annealing schedules, five further loss variants (L1, Charbonnier, L1+SSIM+FFL, NLL+L1, perceptual), and architectural modifications including PReLU activation and Sobel edge supervision. Three curriculum fine-tuning extensions (Arms Q, R, S) additionally demonstrate that two-stage training from a converged L2 checkpoint independently improves perceptual quality (Arm~R: LPIPS~$0.1226$, FSIM~$0.9332$) without retraining from scratch.
...
\begin{itemize}
  ...
  \item \textbf{Chapter 3 (Methods):} Details the Poisson-Gaussian noise model and its
        ...
        uncertainty heads, the four-term composite loss, the 16-arm principal ablation design,
...
Chapter~\ref{chap:lit} begins by establishing the epidemiological case for low-dose imaging and reviewing the technical landscape the proposed framework must navigate.
...
The most capable comparison baselines evaluated in this study are NAFNet and SCUNet, both retrained from scratch on the Kermany dataset under identical Poisson-Gaussian noise conditions. SCUNet was proposed by Zhang et al.\ \cite{zhang_scunet_2023} and introduces the Swin-Conv (SC) block, which processes each feature map along two parallel branches: one branch applies a Swin Transformer attention block for non-local context, and the other applies a residual convolutional block for local detail, before concatenating their outputs. This dual-branch design is the first to explicitly combine local and non-local feature extraction within a single computational block. SCUNet achieved 29.55\,dB on BSD68 at $\sigma = 25$, the highest published value in the standard comparison hierarchy \cite{zhang_scunet_2023}. Critically, SCUNet was also trained using a practical noise synthesis pipeline that includes Poisson noise alongside Gaussian, JPEG compression, and speckle components, making it particularly relevant for the chest X-ray denoising task where Poisson noise dominates at low doses \cite{zhang_scunet_2023, foi_practical_2008}. Table~\ref{tab:lit:architectures} summarises
...
With the clinical motivation established and the technical landscape mapped, Chapter~\ref{chap:methods} specifies the experimental design — the noise model, architecture, and 19-arm ablation study (16 principal arms A–P, plus three curriculum fine-tuning extensions) — that puts these questions to a rigorous test.
...
Table~\ref{tab:results:recon:mid} reports final reconstruction performance on the 624-image held-out test set at mid noise level ($\eta = 200$, $n_{\mathrm{Normal}}=234$, $n_{\mathrm{Pneumonia}}=390$) for all 16 ablation arms, 4 retrained KAIR baselines, DnCNN, BM3D (classical baseline with oracle noise level), and Palette-DDPM (conditional diffusion model, 200-epoch budget). Four metrics are reported: PSNR (pixel fidelity), SSIM (structural similarity), MS-SSIM (multi-scale structural similarity; Wang et al.\ 2003), and LPIPS (learned perceptual image patch similarity via AlexNet; Zhang et al.\ 2018; lower is better). Mean and SD are computed over all 624 test images; 95\% CIs for PSNR are from 1,000 bootstrap resamples. Arms~E, F, G, H, and~P use a VAE bottleneck ($\mathbf{z} = \boldsymbol{\mu}_z$ at inference, deterministic decode); all other arms are fully deterministic. The same results are displayed visually as ranked bar charts in Figures~\ref{fig:results:ablation:psnr} and~\ref{fig:results:ablation:ssim} (Section~\ref{sec:results:ablation}).
...
Three qualitatively distinct failure modes were identified across the ablation arms and three noise levels.
...
Chapter~\ref{chap:discussion} interprets these findings in the context of the prior literature and the clinical mandate, asking not only what the numbers are but what they mean for the design of safe, uncertainty-aware AI systems for paediatric radiology.
...
\textit{Generative baseline.} A Palette-style conditional diffusion model \cite{saharia_palette_2022} trained for 200 epochs under identical GPU budget (12h on an A100) achieved $13.52$~dB PSNR at mid noise on the 624-image test set, approximately $21.1$~dB below Arm~A and $17.2$~dB below BM3D. This result is an honest negative finding rather than an indictment of diffusion modelling as a paradigm: published Palette-style DDPMs are typically trained for thousands of epochs with large batch sizes on cloud-scale hardware \cite{saharia_palette_2022}. The $200$-epoch budget that suffices for all discriminative arms in this study is far too short for the diffusion model's iterative denoising process to converge. The PSNR trajectory was monotonically rising throughout (from $7.1$~dB at epoch~$10$ to $12.9$~dB at epoch~$170$ to $13.5$~dB on the final test evaluation), indicating no asymptote had been approached. This directly supports the efficiency argument for supervised discriminative architectures: PP-VAE-Hformer (Arm~A) achieves $34.62$~dB in $200$ epochs — more than $21$~dB above the diffusion baseline under the same training budget — while simultaneously producing calibrated uncertainty maps unavailable from standard diffusion models.
...
\begin{enumerate}
  ...
  \item \textbf{Uncertainty-guided anatomical hotspot detection.} The per-pixel aleatoric maps $\hat{\sigma}_a$ produced by Arms~D and~S encode spatially varying reconstruction confidence, but their dense pixel format is difficult to action directly in a clinical reporting workflow. A natural translation layer is to aggregate these maps into anatomical \emph{hotspot regions}: contiguous high-uncertainty zones attributed to standard lobar anatomy (right upper lobe, right middle lobe, right lower lobe, left upper lobe, left lower lobe, mediastinum). The proposed pipeline thresholds $\hat{\sigma}_a$ at the 80\textsuperscript{th} percentile of the reconstructed image, applies morphological closing (5$\times$5 structuring element) to merge adjacent uncertain pixels into contiguous blobs, and attributes each blob to an anatomical zone via a heuristic coordinate grid calibrated to the 256$\times$256 CXR field of view. Each hotspot is further characterised by its shape descriptor — \emph{nodular} (eccentricity $< 0.60$, consistent with a mass or consolidation focus), \emph{vascular/linear} (eccentricity $> 0.82$, consistent with a vessel margin or pleural interface), or \emph{irregular} (intermediate), and assigned a severity tier (HIGH if $\bar{\sigma}_a$ exceeds the 90\textsuperscript{th} percentile, MOD otherwise). The output is a per-scan report of the form: ``H1: RLL,HIGH $\bar{\sigma}_a = 0.041$, Nodular,'' directly interpretable without specialist knowledge of the model. Figure~\ref{fig:results:uncertainty:hotspot} demonstrates this pipeline on representative Normal and Pneumonia cases using Arm~S, with the Gaussian-smoothed reconstruction residual $|\text{noisy} - \hat{\mu}|$ as the spatially informative uncertainty proxy (equivalent to what a perfectly calibrated $\hat{\sigma}_a$ approximates). The Normal case produces diffuse vascular/linear hotspots along pleural margins, while the Pneumonia case concentrates hotspots in the lower lobes consistent with the consolidation distribution, exactly the anatomical difference expected from bacterial versus normal lung pathology. Critically, the signal-dependent interpretation (dark parenchyma $\Rightarrow$ high quantum noise $\Rightarrow$ large reconstruction residual $\Rightarrow$ high uncertainty) means these hotspots are not denoiser failures but genuine clinical flags. Calibrating the severity thresholds $\tau_1$, $\tau_2$ against a radiologist reader study with confirmed pathology labels is the natural next step to operationalise this output.
...
Chapter~\ref{chap:conclusion} draws the threads together: restating the principal contributions, answering each research objective directly, and articulating the path from this proof-of-concept to clinical deployment.
...
Three priorities are established by the findings of this study. First, the completed latent space analysis (Section~\ref{sec:results:latent}) confirms that cyclical KL scheduling preserves diagnostically relevant class separation (Arm~F ARI~$=0.333$), but exposes a reconstruction--discrimination trade-off: future work should investigate weakly-supervised or disentangled VAE variants that can achieve both simultaneously. Second, the curriculum fine-tuning results (Arms~R and~S) demonstrate that two-stage training from a strong MSE initialisation is the most effective route to combining pixel fidelity with perceptual quality; extending this curriculum to longer second-stage schedules and additional loss terms (e.g.\ domain-adapted perceptual features from a CXR-pretrained encoder) represents a tractable near-term improvement. Third, a prospective reader study should be conducted to establish whether the aleatoric uncertainty maps of Arms~D and~S translate to measurable improvements in radiologist diagnostic confidence on real low-dose CXR acquisitions — the critical experiment that would convert this proof-of-concept into a clinically validated tool.
