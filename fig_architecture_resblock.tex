...
% Figure 2 -- ResBlock (convolutional block) detail
...
\begin{tikzpicture}[
  ...
  addnode/.style={draw=gray!65, fill=gray!13, circle,
                  minimum size=0.65cm, align=center, line width=0.8pt,
                  font=\normalsize},
...
% Same sample image for output (shows identity-preserving residual)
...
\begin{scope}[shift={(3.2, 5.0)}]
  ...
  \begin{scope}[shift={(1.95,0.0)}]
    ...
    \node[font=\fontsize{5}{5}\selectfont, white, below=1pt] at (0.375,-0.01)
      {kernel};
  \end{scope}
  % Labels (white on blue background)
  ...
  % Pointer arrow
  \draw[->, white, line width=0.8pt] (-0.5,1.4) -- (0.45,1.1);
  \node[font=\fontsize{7}{8}\selectfont, white, left=2pt] at (-0.5,1.4)
    {receptive field};
\end{scope}

% Dashed connector from conv1 to annotation panel
\draw[gray!50, dashed, line width=0.6pt] (conv1.east) -- (3.2, 6.7);
