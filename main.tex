...
Table~\ref{tab:methods:noise_presets} characterises the three noise presets used throughout this study. The effective noise standard deviation $\sigma_{\mathrm{eff}}$ at a pixel with mean intensity $\bar{y}=0.5$ is $\sigma_{\mathrm{eff}} = \sqrt{a \cdot \bar{y} + b}$, yielding $\sigma_{\mathrm{eff}} \approx 0.074$, $0.122$, and $0.204$ for Low, Mid, and High presets respectively (equivalent to $\sigma \approx 15$, $25$, and $50$ in the $[0,255]$ scale used by conventional denoising benchmarks). The corresponding noisy-to-clean PSNR of the degraded input — before any model inference — is approximately $28.3$~dB, $25.4$~dB, and $21.5$~dB at the three levels, confirming that the High preset represents a clinically extreme dose reduction that would be unacceptable in routine practice without restoration.

The noise variance is intentionally signal-dependent: a pixel at intensity $0.1$ (deep shadow) has effective $\sigma \approx 0.19$ at the Mid preset, while a pixel at intensity $0.9$ (bright aerated lung) has $\sigma \approx 0.12$. This $1.6\times$ variance ratio across the intensity range confirms that dark anatomical regions (pleural angles, retrocardiac space) are substantially harder to denoise than bright regions, and that per-pixel uncertainty maps ($\hat{\sigma}_a$) should — and empirically do — show elevated uncertainty in these dark regions.
...
\figplaceholder{Representative paediatric CXR pairs at three Poisson-Gaussian degradation levels (Low/Mid/High presets; $\sigma_{\mathrm{eff}}\approx15/25/50$). Left column: clean reference. Remaining columns: noisy input at each preset.}

\tabplaceholder{Quantitative noise characterisation: mean noisy-image PSNR, SNR, and pixel intensity statistics per noise preset. Normal vs.\ Pneumonia subgroups.}
...
\caption[Qualitative reconstruction -- Arm E (PP-VAE), Bacterial Pneumonia cases]{
  Arm~E (PP-VAE, linear KL; $31.635$~dB) on three Bacterial Pneumonia cases.
  \textit{Columns:} Noisy | Reference | $\hat{\mu}$ | $\hat{\sigma}_a$ | $|\hat{\mu}-y|$ | Epist.\ $\hat{\sigma}_e$.
  The VAE bottleneck imposes a $1.8$~dB PSNR cost relative to Arm~D, visible as elevated reconstruction error at consolidation margins. The aleatoric head is well-calibrated but the linear KL schedule leads to partial posterior collapse ($\hat{\sigma}_e$ maps are compressed relative to Arm~H).}
\label{fig:results:qual:e:bacterial}
\end{figure}
...
\caption[Qualitative reconstruction -- Arm E (PP-VAE), Normal cases]{
  Arm~E on three Normal cases. Uncertainty maps remain anatomically plausible despite the KL regularisation cost, with boundary-localised $\hat{\sigma}_a$ analogous to Arm~D (Figure~\ref{fig:results:qual:d:normal}). Epistemic uncertainty $\hat{\sigma}_e$ shows limited class discrimination under linear KL.}
...
\centering
\evalfig{figures/evaluation_v2/uncertainty_arm_d_nll_ssim_ffl}
\caption[Uncertainty calibration panel -- Arm D]{%
  Pixel-level aleatoric uncertainty maps for Arm~D (NLL+SSIM+FFL) at mid noise ($\eta=200$),
  four Bacterial Pneumonia cases.
  \textit{Columns:} Noisy | Reference | $\hat{\mu}$ | $\hat{\sigma}_a$ (inferno, auto-scaled) | $|\hat{\mu}-y|$.
  The colourmap range is set to the 99th-percentile of $\hat{\sigma}_a$ per panel, revealing
  the well-calibrated sub-0.03 aleatoric spread. Higher $\hat{\sigma}_a$ concentrates at
  consolidation margins and cardiac silhouette, confirming near-diagonal calibration between
  predicted uncertainty and observed reconstruction error.}
...
\figplaceholder{Sensitivity of PSNR to perceptual loss weight $\lambda_{\mathrm{perc}}$ and edge loss weight $\lambda_{\mathrm{edge}}$ in PP-VAE: future work not yet conducted. Estimated surface shown for illustrative purposes only.}
...
\figplaceholder{Reconstruction quality (PSNR, SSIM) and latent separation (silhouette score) as a function of latent channel dimension $C_z \in \{16, 32, 64, 128\}$: future work.}
...
\begin{table}[htbp]
\centering
\caption[Bacterial vs.\ Viral Pneumonia subgroup PSNR]{%
  Reconstruction PSNR (dB) stratified by Bacterial vs.\ Viral Pneumonia subgroup at mid noise
  ($\eta=200$, mean\,$\pm$\,SD). $\Delta_{\mathrm{B-N}}$ = Bacterial $-$ Normal;
  $\Delta_{\mathrm{V-N}}$ = Viral $-$ Normal.
  Bacterial cases ($n=242$) consistently outperform Viral ($n=148$) by $\approx$0.5--0.6~dB
  across all arms, attributable to the higher local contrast of lobar consolidation
  vs.\ diffuse interstitial pattern.
  Full 21-model breakdown pending \texttt{compute\_subgroup.sh} (see §\ref{sec:results:subgroup}).}
...
\midrule
\multicolumn{6}{l}{\textit{Full 16-arm + 5-baseline breakdown: run \texttt{sbatch slurm/compute\_subgroup.sh} on CSD3}} \\
\bottomrule
\end{tabular}
\end{table}
...
\figplaceholder{Failure case gallery: three cases for each failure mode (Arm~O PReLU artefacts, Arm~E post-collapse, Arm~M interference) with reconstructed $\hat{\mu}$, error map $|\hat{\mu}-y|$, and $\hat{\sigma}_a$ where available.}
...
The computational work in this study used Anthropic Claude Code (claude-sonnet-4-6) for code generation assistance in LaTeX typesetting, SLURM job scripts, and Python analysis scripts. All model training, evaluation, and scientific conclusions are the work of the author. AI-generated code was reviewed and verified before use. No AI-generated text was used in the scientific analysis or interpretation sections of this dissertation without disclosure.
