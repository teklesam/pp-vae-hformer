...
Figure~\ref{fig:results:training} shows the validation PSNR trajectory for Arms~B, C, and~D over 200 training epochs (Arms~A and~E training logs are presented once final checkpoints are downloaded; the KL divergence panel will be populated from Arm~E). All three arms converged smoothly without divergence events. Validation PSNR was computed every five epochs using a deterministic forward pass; the interleaved \texttt{nan} entries in the training logs between evaluation epochs are by design and do not indicate instability.
...
Figures~\ref{fig:results:qual:pneumonia} and~\ref{fig:results:qual:normal} present qualitative reconstructions for three representative Pneumonia and three Normal cases at mid noise level. Each panel shows: noisy input, clean target, the model's denoised mean $\hat{\mu}$, the aleatoric uncertainty map $\hat{\sigma}_a = \exp(0.5\,\hat{\ell}_{\sigma^2})$, and the pixel-wise absolute error $|\hat{\mu} - y|$. The epistemic uncertainty $\hat{\sigma}_e$ from Monte Carlo sampling is shown only for Arm~E.

\begin{figure}[H]
\centering
\evalfig{figures/evaluation/qualitative_arm_d_nll_ssim_ffl_pneumonia}
\caption[Qualitative reconstruction -- Pneumonia cases]{
  Qualitative reconstruction examples for three representative Pneumonia test cases at mid noise level using Arm~D (Hformer + NLL + SSIM + FFL), the best-performing fully-evaluated arm.
  \textit{Left to right:} noisy input, clean target, denoised mean ($\hat{\mu}$), aleatoric uncertainty ($\hat{\sigma}_a$, inferno colourmap), absolute reconstruction error ($|\hat{\mu}-y|$, RdBu colourmap).
  Elevated aleatoric uncertainty at consolidated lung regions is consistent with genuine image ambiguity at pathological sites.
  Epistemic uncertainty maps ($\hat{\sigma}_e$, Arm~E only) will be added upon training completion.}
...
\caption[Qualitative reconstruction -- Normal cases]{
  Qualitative reconstruction examples for three representative Normal (healthy) test cases at mid noise level using Arm~D (Hformer + NLL + SSIM + FFL).
  \textit{Left to right:} noisy input, clean target, denoised mean ($\hat{\mu}$), aleatoric uncertainty ($\hat{\sigma}_a$, inferno colourmap), absolute reconstruction error ($|\hat{\mu}-y|$, RdBu colourmap).
  In Normal cases, aleatoric uncertainty concentrates at anatomical boundaries (cardiac silhouette, diaphragm) rather than being diffusely elevated across the lung field, in contrast to the Pneumonia cases in Figure~\ref{fig:results:qual:pneumonia}.
  Epistemic uncertainty maps ($\hat{\sigma}_e$, Arm~E only) will be added upon training completion.}
...
Figure~\ref{fig:results:dosecurve} shows PSNR and SSIM as a function of noise severity for all arms. All arms degrade monotonically from low to high noise, as expected. The relative ordering of arms is preserved across all noise levels (Arm~A $>$ Arm~D $\approx$ Arm~B $>$ Arm~C), indicating that the loss-function ranking is robust to degradation severity. The gap between Arm~C (NLL+SSIM) and the remaining arms widens slightly at high noise, suggesting that SSIM-only structural regularisation becomes less effective at suppressing the increased stochastic fluctuations in the Poisson-Gaussian degradation model. Full cross-noise statistics including Arm~E and DnCNN will be reported upon completion of training.
