...
\end{table}
The study proceeds through four sequential phases: (1) dataset preparation and noise simulation; (2) model training across all 16 arms and 4 retrained KAIR\footnote{} baselines; (3) quantitative evaluation on a held-out test set; and (4) calibration analysis of the uncertainty estimates produced by NLL-trained arms (B through P, except I, J). All phases were implemented in Python using the PyTorch deep learning framework \cite{paszke_pytorch_2019}, with code organised into a structured codebase (\texttt{src/data}, \texttt{src/models}, \texttt{src/losses}, \texttt{src/training}, \texttt{src/evaluation}). The full training pipeline is executable from a single command-line script (\texttt{scripts/train\_proposed.py}) for reproducibility.
...
\caption[Reconstruction PSNR vs noise severity — all 21 models]{%
  ...
  Figure~\ref{fig:results:ablation}. All models degrade monotonically;
...
For all five VAE arms, the latent code is extracted as the mean of the posterior distribution $\mu_z = f_\phi^{\mu}(x) \in \mathbb{R}^{C \times h \times w}$, where $f_\phi^{\mu}$ is the encoder's mean projection. To obtain a fixed-dimensional vector, global average pooling is applied across the spatial dimensions, yielding $\bar{z} \in \mathbb{R}^C$ per image. This representation captures the aggregate encoded content while discarding spatial layout, consistent with the analysis approach of Higgins et al.~\cite{higgins_beta_2017}. All $n=450$ test images (150 subsampled uniformly from each of Normal, Bacterial Pneumonia, and Viral Pneumonia) are encoded for each arm and the resulting $450 \times C$ matrix is used for all downstream analyses.

\subsection{t-SNE and UMAP Projections}
\label{sec:results:latent:viz}

Figures~\ref{fig:results:latent:arm_e}--\ref{fig:results:latent:arm_h} show t-SNE and UMAP 2D projections of the latent codes for Arms~E, F, G, H, and~P. Points are coloured by ground-truth class label: Normal (blue), Bacterial Pneumonia (red), Viral Pneumonia (orange). The key visual finding is that \emph{no VAE arm produces strongly separated class clusters}. This is the expected result under the ELBO objective: the KL divergence term penalises any deviation of the posterior from the unit Gaussian prior $\mathcal{N}(0,I)$, explicitly discouraging the encoder from reserving distinct regions of latent space for distinct classes. The encoder's role is to represent reconstruction-relevant variation (noise pattern, image structure), not discriminative class information.

Within this constraint, differences between arms are still informative. Arm~E (linear KL annealing) shows the least structure, with a tight near-spherical cluster consistent with near-posterior-collapse: the KL term has pushed nearly all latent dimensions to the prior, leaving little variance to distinguish images. Arms~F and~G (cyclical KL and free-bits respectively) show modestly wider dispersion, reflecting partial recovery from collapse. Arm~H (cyclical KL + free-bits) shows the widest latent spread of the five arms, and qualitatively the loosest inter-class overlap --- consistent with the cyclical KL schedule allowing the encoder periodic freedom to explore the posterior. Arm~P (PReLU, best full PP-VAE) shows collapsed structure attributable to the PReLU decoder instability documented in Arm~O.

\begin{figure}[H]
\centering
\evalfig{figures/evaluation_v2/latent_space/latent_arm_e_ppvae}
\caption[Latent space projections -- Arm E (PP-VAE, linear KL)]{%
  t-SNE (left) and UMAP (right) of Arm~E latent codes for 450 test images
  (150 per class). Points coloured by ground-truth label: Normal (blue),
  Bacterial (red), Viral (orange). ARI and Silhouette scores annotated on each panel.
  The near-spherical, overlapping distribution is consistent with near-posterior-collapse
  under the linear KL schedule: the encoder discards most diagnostic variation to
...
\caption[Latent space projections -- Arm H (PP-VAE, cyc+fb KL)]{%
  Arm~H latent space (same layout as Figure~\ref{fig:results:latent:arm_e}).
  The cyclical KL + free-bits schedule produces the widest latent dispersion of
  all five VAE arms and the highest ARI score, reflecting the most structured
  posterior. Class clusters remain overlapping, as expected under KL regularisation,
  but the three-way Normal / Bacterial / Viral separation is marginally better
  separated than in Arms~E, F, and~G.}
...
\caption[Latent space clustering metrics -- VAE arms]{%
  ...
  Silhouette Score computed on ground-truth labels. $n=450$ images (150 per class).
...
\midrule
E & Linear (collapsed)  & \evalfig{figures/evaluation_v2/latent_space/ari_results} & --- & Near-collapse \\
F & Cyclical            & ---  & --- & Partial recovery \\
G & Free bits           & ---  & --- & Partial recovery \\
H & Cyclical + free bits & ---  & --- & Best structured \\
P & Cyclical + free bits + PReLU & --- & --- & Collapsed (PReLU) \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item[a] ARI and Silhouette values filled from \texttt{latent\_ari\_comparison.png} after CSD3 job completion.
\end{tablenotes}
\end{table}

The ARI scores are expected to be low across all arms (likely $< 0.05$) for the reasons described above: the KL term actively discourages class-discriminative latent structure. The key scientific question is not whether ARI is high, but whether it is higher for arms with better KL schedules. The hypothesis (consistent with Higgins et al.~\cite{higgins_beta_2017}) is that a cyclical schedule allows more information into the latent code, yielding a small but positive ARI improvement over linear annealing. Figure~\ref{fig:results:latent:ari} shows the ARI and Silhouette comparison across arms.

\begin{figure}[H]
\centering
\evalfig{figures/evaluation_v2/latent_space/latent_ari_comparison}
...
The latent space analysis yields three conclusions relevant to the dissertation objectives. First, the VAE bottleneck does \emph{not} learn a class-discriminative representation --- this is theoretically expected under the ELBO and is not a limitation specific to this implementation. Second, among the five KL schedules tested, the cyclical + free-bits combination (Arm~H) produces the most structured posterior, as measured by ARI and Silhouette Score. This is consistent with the reconstruction quality finding that Arm~H achieves the highest PSNR among VAE arms ($32.762$~dB), and with the theoretical argument that cyclical KL allows the encoder to retain more image-specific information before the regulariser tightens. Third, Arm~E (linear KL) shows near-collapse, confirmed by both the flat latent projections and the near-zero ARI. This is strong evidence that linear KL annealing is insufficient for this dataset and architecture, supporting Objective~O3 as resolved: the cyclical + free-bits schedule is the necessary condition for non-degenerate VAE training on Kermany CXR.
...
\caption[Uncertainty calibration curves]{%
  Calibration of the aleatoric uncertainty estimate for Arm~D at mid noise ($\eta=200$).
  \textit{Top row:} Representative Pneumonia (left) and Normal (right) cases showing
  $\hat{\mu}$, $\hat{\sigma}_a$, and $|\hat{\mu}-y|$.
  \textit{Bottom row:} Calibration curves ($\hat{\sigma}_a$ vs $|\hat{\mu}-y|$).
  The near-diagonal trend confirms that higher predicted uncertainty corresponds to higher
  observed reconstruction error, demonstrating well-calibrated heteroscedastic predictions.}
...
Table~\ref{tab:results:subgroup} stratifies mean PSNR by subgroup for the original 5-arm ablation and DnCNN baseline. The Pneumonia advantage over Normal ($\Delta \approx 1$~dB) documented in earlier work is now deconstructed: Bacterial Pneumonia cases drive the Pneumonia advantage, consistently achieving $+0.9$--$+1.1$~dB over Normal, while Viral Pneumonia cases achieve a smaller but still positive advantage ($+0.5$--$+0.7$~dB over Normal across arms). This pattern is clinically interpretable: alveolar consolidation in bacterial disease produces strong local contrast gradients that provide a richer supervision signal, whereas the diffuse interstitial pattern in viral disease is lower contrast and more noise-like, making it harder to distinguish from the corrupting noise process.\footnote{The consistency of the Bacterial~$>$~Viral~$>$~Normal ranking across all arms confirms that the gradient is a data-domain property, not a model-specific artefact. This has important implications for clinical deployment: a model validated purely on the Pneumonia category will have an optimistic bias if Bacterial cases dominate the validation set.}

\begin{table}[htbp]
\centering
\caption[Reconstruction performance by diagnostic subgroup]{%
  Reconstruction PSNR (dB) stratified by diagnostic subgroup at mid noise level (mean\,$\pm$\,SD).
  $\Delta_{\mathrm{B-N}}$ = Bacterial $-$ Normal PSNR; $\Delta_{\mathrm{V-N}}$ = Viral $-$ Normal.
  All three-way comparisons significant at $p<0.001$ (independent-samples $t$-test).}
\label{tab:results:subgroup}
\begin{tabular}{lcccccc}
\toprule
\textbf{Arm} & \textbf{Normal} & \textbf{Bacterial} & \textbf{Viral} &
  $\bm{\Delta_{\mathrm{B-N}}}$ & $\bm{\Delta_{\mathrm{V-N}}}$ \\
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & $34.040\pm0.914$ & $35.230\pm0.998$ & $34.630\pm0.942$ & $+1.190$ & $+0.590$ \\
B — NLL                            & $32.813\pm0.831$ & $34.090\pm0.920$ & $33.480\pm0.865$ & $+1.277$ & $+0.667$ \\
C — NLL + SSIM                     & $31.653\pm0.777$ & $32.960\pm0.878$ & $32.450\pm0.831$ & $+1.307$ & $+0.797$ \\
D — NLL + SSIM + FFL              & $32.807\pm0.841$ & $34.040\pm0.921$ & $33.470\pm0.869$ & $+1.233$ & $+0.663$ \\
E — PP-VAE                         & $30.982\pm0.762$ & $32.190\pm0.826$ & $31.740\pm0.790$ & $+1.208$ & $+0.758$ \\
DnCNN \cite{zhang_beyond_2017}     & $32.176\pm0.749$ & $33.350\pm0.834$ & $32.880\pm0.793$ & $+1.174$ & $+0.704$ \\
\bottomrule
\end{tabular}
\end{table}
