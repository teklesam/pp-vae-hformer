...
where $\alpha$ governs signal-dependent Poisson noise and $\beta$ the signal-independent electronic noise floor. As dose decreases, the signal $y(x)$ falls and the image enters photon starvation: Poisson fluctuations become visible and $\beta$ begins to overwhelm the signal \cite{foi_practical_2008}.
...
The Variational Autoencoder (VAE) \cite{kingma_auto-encoding_2014} offers a probabilistic alternative: the encoder maps input $x$ to a posterior $q_\phi(z|x)$ in a latent space, enabling pixel-wise epistemic uncertainty maps via repeated sampling. However, the standard ELBO's factorised Gaussian likelihood reduces to unweighted MSE, producing blur, and the KL penalty can force the posterior to collapse to the prior, causing the model to output a generic average image regardless of patient anatomy \cite{bredell_explicitly_2023, zhao_loss_2017, lucas_dont_2019}.
