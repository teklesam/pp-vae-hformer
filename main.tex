...
inductive bias and failure mechanism. \textit{Discriminative supervised methods}
...
\begin{table}[H]
  \centering
  \caption{Comparative analysis of image denoising method classes across clinically relevant
  axes. Loss abbreviations: MSE = mean squared error; Charb. = Charbonnier; NLL = heteroscedastic
  negative log-likelihood. Noise model: AWGN = additive white Gaussian noise;
  P-G = Poisson-Gaussian. UQ = uncertainty quantification. The PP-VAE-Hformer is the
  only method that addresses all six identified deficiencies simultaneously.}
  \label{tab:intro:comparison}
  \footnotesize
  \begin{tabularx}{\linewidth}{p{1.9cm} p{2.1cm} p{1.2cm} p{1.2cm} p{1.1cm} p{2.9cm} p{2.6cm} p{0.8cm} p{0.8cm}}
    \toprule
    \textbf{Method Class} &
    \textbf{Key Examples} &
    \textbf{Domain} &
...
% ─── Architecture Evolution Figure ────────────────────────────────────────────
\begin{landscape}
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    era/.style={font=\footnotesize\bfseries, text=white, fill=gray!60, rounded corners=3pt,
                minimum width=1.5cm, minimum height=0.6cm, align=center},
    classical/.style={draw=blue!50!black, fill=blue!8, rounded corners=4pt,
                      text width=2.6cm, align=center, inner sep=4pt, minimum height=1.5cm},
    ...
    proposed/.style={draw=teal!70!black, fill=teal!12, rounded corners=4pt,
                     text width=3.0cm, align=center, inner sep=4pt, minimum height=1.5cm,
                     very thick, drop shadow},
    gap/.style={draw=red!70, fill=red!5, dashed, rounded corners=3pt,
                text width=2.2cm, align=center, inner sep=3pt, font=\tiny},
    >=Stealth,
    arr/.style={->, thick, darkblue!70},
    harr/.style={->, thick, red!70, dashed},
  ]

  %% ERA LABELS (column headers)
  \node[era] at (0,    7.2) {Pre-2017};
  \node[era] at (3.1,  7.2) {2017};
  \node[era] at (6.2,  7.2) {2018--2019};
  \node[era] at (9.3,  7.2) {2021};
  \node[era] at (12.4, 7.2) {2022--2023};
  \node[era, fill=teal!60] at (15.5, 7.2) {Proposed};

  %% CLASSICAL METHODS
  \node[classical] (bm3d) at (0, 5.5)
    {\textbf{BM3D}\\\textit{Dabov 2007}\\Filter-based\\28.57\,dB\\\textsc{no training}};
  \node[classical] (nlm) at (0, 3.5)
    {\textbf{NLM}\\\textit{Buades 2005}\\Self-similarity\\Over-smooths};
  \node[classical] (tv) at (0, 1.5)
    {\textbf{TV / Wavelet}\\\textit{Rudin 1992}\\Variational\\Loses texture};

  %% CNN BASIC — 2017
  \node[cnnl2] (dncnn) at (3.1, 6.0)
    {\textbf{DnCNN}\\\textit{Zhang 2017}\\Residual CNN\\L2 loss\\29.23\,dB};
  \node[cnnl2] (redcnn) at (3.1, 3.8)
    {\textbf{RED-CNN}\\\textit{Chen 2017}\\Enc-Dec $\bullet$ CT\\L2 loss\\Med.\ baseline};
  \node[cnnl2] (ircnn) at (3.1, 1.8)
    {\textbf{IRCNN}\\\textit{Zhang 2017}\\Plug-and-play\\L2 loss\\29.15\,dB};

  %% CNN ADVANCED — 2018-2019
  \node[cnnl2] (ffdnet) at (6.2, 6.8)
    {\textbf{FFDNet}\\\textit{Zhang 2018}\\$\sigma_M$-cond.\\L2 loss\\29.19\,dB};
  \node[cnnl2] (n3net) at (6.2, 5.2)
    {\textbf{N3Net}\\\textit{Pl\"{o}tz 2018}\\Learned KNN\\L2 loss\\29.30\,dB};
  \node[cnnl2] (mwcnn) at (6.2, 3.6)
    {\textbf{MWCNN}\\\textit{Liu 2018}\\Wavelet pool\\L2 loss\\29.41\,dB};
  \node[cnnl2] (rnan) at (6.2, 2.0)
    {\textbf{RNAN}\\\textit{Zhang 2019}\\Non-local attn\\L2 loss\\29.44\,dB};
  \node[cnnl2] (focnet) at (6.2, 0.4)
    {\textbf{FOCNet}\\\textit{Jia 2019}\\Frac.\ ODE\\L2 loss\\29.38\,dB};

  %% CNN DEEP + DENSE — 2021
  \node[cnnl2] (drunet) at (9.3, 6.5)
    {\textbf{DRUNet}\\\textit{Zhang 2021}\\Deep U-Net\\L2 $\bullet$ 32.7M\\\textbf{CNN ceil.\ 29.48\,dB}};
  \node[charb] (uddn) at (9.3, 4.3)
    {\textbf{UDDN}\\\textit{Luo 2021}\\Dense CNN\\Charbonnier\\\textit{Cardiac X-ray}};

  %% TRANSFORMER / HYBRID — 2021-2023
  \node[transformer] (swinir) at (12.4, 6.5)
    {\textbf{SwinIR}\\\textit{Liang 2021}\\Swin Transformer\\Charbonnier\\29.50\,dB};
  \node[charb] (eedn) at (9.3, 2.3)
    {\textbf{EEDN}\\\textit{Luo 2022}\\Dense+Edge\\Charbonnier\\\textit{Cardiac X-ray}};
  \node[transformer] (nafnet) at (12.4, 4.7)
    {\textbf{NAFNet}\\\textit{Chen 2022}\\SimpleGate\\MAE loss\\$\approx$29.52\,dB};
  \node[transformer] (scunet) at (12.4, 3.0)
    {\textbf{SCUNet}\\\textit{Zhang 2023}\\SC block\\\textbf{Incl.\ Poisson noise}\\\textbf{29.55\,dB (ceiling)}};
  \node[cnnl2] (hformer) at (12.4, 1.3)
    {\textbf{Hformer}\\\textit{Zhang 2023}\\DWConv+LSA\\\textbf{L2 loss (gap!)}\\1.65M $\bullet$ CT};

  %% GENERATIVE (side branch, shown as warning)
  \node[generative] (gan) at (6.2, -1.5)
    {\textbf{GAN}\\\textit{Goodfellow 2014}\\Adversarial\\Hallucination risk};
  \node[generative] (diffusion) at (9.3, -1.5)
    {\textbf{Diffusion}\\\textit{Ho 2020}\\Iterative sampling\\Pfaff (2024):\\\textbf{pathology erased}};

  %% PROPOSED
  \node[proposed] (proposed) at (15.5, 4.0)
    {\textbf{PP-VAE-Hformer}\\(This work)\\\rule{2.5cm}{0.2pt}\\Hformer + VAE\\NLL+SSIM+FFL+KL\\Paed.\ CXR $\bullet$ P-G noise\\$\hat{\mu}$ + $\hat{\sigma}^2_a$ + $\hat{\sigma}^2_e$};

  %% EVOLUTION ARROWS — CNN lineage
  \draw[arr] (bm3d.east) -- (dncnn.west);
  \draw[arr] (dncnn.east) -- (ffdnet.west);
  \draw[arr] (dncnn.east) -- (n3net.west);
  \draw[arr] (dncnn.east) -- (mwcnn.west);
  \draw[arr] (dncnn.east) -- (rnan.west);
  \draw[arr] (dncnn.east) -- (focnet.west);
  \draw[arr] (ircnn.east) -- (ffdnet.south west);
  \draw[arr] (redcnn.east) -- (uddn.west);
  \draw[arr] (mwcnn.east) to[bend left=10] (drunet.west);
  \draw[arr] (rnan.east) to[bend right=10] (drunet.south west);
  \draw[arr] (ffdnet.east) -- (drunet.west);
  \draw[arr] (drunet.east) -- (swinir.west);
  \draw[arr] (drunet.east) -- (nafnet.west);
  \draw[arr] (drunet.east) -- (scunet.west);
  \draw[arr] (uddn.east) -- (eedn.west);
  \draw[arr] (eedn.east) to[bend right=15] (hformer.west);
  \draw[arr] (swinir.east) to[bend left=15] (proposed.north west);
  \draw[arr] (hformer.east) -- (proposed.west);
  \draw[arr] (scunet.east) to[bend right=15] (proposed.south west);

  %% GENERATIVE (warning arrows)
  \draw[harr] (gan.east) -- (diffusion.west);
  \draw[harr, ->] (diffusion.east) to[bend right=20]
      node[midway, above, font=\tiny, text=red!70] {\texttimes\ Hallucination} (hformer.south);

  %% LEGEND
  \begin{scope}[shift={(0, -3.5)}]
    \node[cnnl2, minimum width=1.5cm, minimum height=0.5cm, text width=1.5cm]
      at (0, 0) {\textbf{L2 (MSE) loss}};
    \node[charb, minimum width=1.8cm, minimum height=0.5cm, text width=1.8cm]
      at (3.0, 0) {\textbf{Charbonnier loss}};
    \node[transformer, minimum width=1.8cm, minimum height=0.5cm, text width=1.8cm]
      at (6.2, 0) {\textbf{Hybrid Transformer}};
    \node[classical, minimum width=1.6cm, minimum height=0.5cm, text width=1.6cm]
      at (9.3, 0) {\textbf{Classical method}};
    \node[proposed, minimum width=1.6cm, minimum height=0.5cm, text width=1.6cm]
      at (12.4, 0) {\textbf{Proposed}};
    \node[font=\tiny, text=red!70] at (15.5, 0) {\texttimes\ Hallucination risk};
  \end{scope}

  \end{tikzpicture}
  \caption{Evolution of image denoising architectures from classical methods (2007) through
  to the proposed PP-VAE-Hformer. Arrows indicate conceptual inheritance or direct extension.
  Colour indicates loss function family: red = MSE/L2 (the dominant paradigm throughout the
  progression); orange = Charbonnier (partial improvement); purple = hybrid Transformer;
  teal = proposed composite NLL + MS-SSIM + FFL + KL. Every method in the progression
  uses AWGN noise assumptions and produces no uncertainty output; the proposed
  framework is the first to address Poisson-Gaussian noise physics, paediatric CXR, and
  dual aleatoric/epistemic uncertainty simultaneously. DnCNN (2017) is the ancestral baseline
  from which all subsequent CNN methods descend; Hformer (2023) is the proposed backbone,
  retained for its efficiency (1.65\,M parameters) but trained with the novel composite loss.}
  \label{fig:intro:evotree}
\end{figure}
\end{landscape}
...
\begin{table}[H]
  \centering
  \caption{Radiation dose and attributable cancer risk by imaging modality in children, from
  Smith-Bindman et al.\ \cite{smith-bindman_hematologic_2025}. The contrast between chest
  radiography and CT of the head illustrates why preventing escalation from X-ray to CT is
  clinically meaningful for cancer risk.}
  \label{tab:lit:dose}
  \small
...
Image denoising has a long research history, and the field has undergone rapid change in the
...
\begin{table}[H]
  \centering
  \caption{PSNR (dB) on the BSD68 greyscale denoising benchmark at three standard noise
  levels, showing the full chronological progression from classical methods to the current
  state of the art. All numbers from Liang et al.\ \cite{liang_swinir_2021} Table~5 unless
  noted. \dag: from Zhang et al.\ \cite{zhang_scunet_2023} Table~1. Higher PSNR is better.
  The architecture tier and dominant loss are highlighted to show that performance gains
  have come primarily from architectural innovation, not loss function improvement: the field
  has used L2 or Charbonnier throughout its entire development.}
  ...
  \begin{tabularx}{\textwidth}{l l l l r r r}
    ...
    BM3D \cite{dabov_image_2007}           & 2007 & Classical     & n/a         & 31.73 & 28.57 & 25.62 \\
    WNNM                                   & 2014 & Classical     & n/a         & 31.37 & 29.41 & 25.87 \\
    \midrule
    \multicolumn{7}{l}{\textit{Tier 1 -- CNN: Basic residual}} \\
    DnCNN \cite{zhang_beyond_2017}         & 2017 & CNN-basic     & \textbf{L2} & 31.73 & 29.23 & 26.23 \\
    IRCNN \cite{zhang_drunet_2021}         & 2017 & CNN-P\&P      & \textbf{L2} & 31.63 & 29.15 & 26.22 \\
    \midrule
    \multicolumn{7}{l}{\textit{Tier 2 -- CNN: Explicit noise conditioning}} \\
    FFDNet \cite{zhang_drunet_2021}        & 2018 & CNN-$\sigma$  & \textbf{L2} & 31.63 & 29.19 & 26.29 \\
    \midrule
    \multicolumn{7}{l}{\textit{Tier 3 -- CNN: Non-local and frequency-domain}} \\
    N3Net \cite{zhang_drunet_2021}         & 2018 & CNN-nonlocal  & \textbf{L2} & 31.78 & 29.30 & 26.39 \\
    MWCNN \cite{zhang_drunet_2021}         & 2018 & CNN-wavelet   & \textbf{L2} & 31.86 & 29.41 & 26.53 \\
    FOCNet \cite{jia2019focnet}            & 2019 & CNN-math      & \textbf{L2} & 31.83 & 29.38 & 26.50 \\
    RNAN \cite{zhang_drunet_2021}          & 2019 & CNN-attn      & \textbf{L2} & 31.86 & 29.44 & 26.48 \\
    \midrule
    \multicolumn{7}{l}{\textit{Tier 4 -- CNN: Deep and conditioned (ceiling)}} \\
    DRUNet \cite{zhang_drunet_2021}        & 2021 & CNN-deep      & \textbf{L2} & \textbf{31.91} & \textbf{29.48} & \textbf{26.59} \\
    \midrule
    \multicolumn{7}{l}{\textit{Tier 5 -- Transformer / Hybrid (current state of the art)}} \\
    SwinIR \cite{liang_swinir_2021}        & 2021 & Transformer   & Charbonnier & 31.97 & 29.50 & 26.58 \\
    NAFNet \cite{chen_nafnet_2022}         & 2022 & Conv no-act.  & MAE         & 31.99 & $\approx$29.52 & --- \\
    SCUNet \cite{zhang_scunet_2023}        & 2023 & SC-hybrid\dag & \textbf{L2} & \textbf{31.99} & \textbf{29.55} & \textbf{26.67} \\
    Hformer \cite{zhang_hformer_2023}      & 2023 & CNN+attn      & \textbf{L2} & --- & --- & --- \\
    \midrule
...
\begin{table}[H]
  \centering
  \caption{Cross-cutting gaps across all major denoising methods in the comparison hierarchy.
  Every column represents a gap that the dissertation addresses; every row shows a method that
  fails to fill it. Tick marks (\checkmark) indicate the gap is addressed; crosses (\texttimes)
  indicate it is not. Partial ticks (P) indicate a partial or indirect address. The proposed
  PP-VAE-Hformer is the only method to address all six gaps simultaneously. Domain
  abbreviations: CXR = chest X-ray, CT = computed tomography, Nat. = natural images.}
  \label{tab:lit:gaps}
  \small
  \begin{tabularx}{\textwidth}{l l l X X X X}
    ...
    \textbf{Domain} &
    \textbf{Noise model} &
    \textbf{Structural loss} &
    \textbf{Freq.\ loss} &
    \textbf{UQ output} &
    \textbf{Paed.\ CXR} \\
    \midrule
    RED-CNN \cite{chen_low-dose_2017}    & CT      & Gaussian & \texttimes & \texttimes & \texttimes & \texttimes \\
    UDDN \cite{luo_uddn_2021}           & Cardiac CXR & Gaussian & \texttimes & \texttimes & \texttimes & \texttimes \\
    EEDN \cite{luo_eedn_2022}           & Cardiac CXR & Gaussian & P (arch.)  & P (arch.)  & \texttimes & \texttimes \\
    DRUNet \cite{zhang_drunet_2021}     & Nat.    & Gaussian & \texttimes & \texttimes & \texttimes & \texttimes \\
    SwinIR \cite{liang_swinir_2021}     & Nat.    & Gaussian & \texttimes & \texttimes & \texttimes & \texttimes \\
    NAFNet \cite{chen_nafnet_2022}      & Nat.    & Real/AWGN & \texttimes & \texttimes & \texttimes & \texttimes \\
    SCUNet \cite{zhang_scunet_2023}     & Nat.    & P-G+JPEG & \texttimes & \texttimes & \texttimes & \texttimes \\
    Hformer \cite{zhang_hformer_2023}   & CT      & Gaussian & \texttimes & \texttimes & \texttimes & \texttimes \\
    \midrule
    \textbf{PP-VAE-Hformer (proposed)}  & \textbf{Paed.\ CXR} & \textbf{Poisson-Gaussian} &
...
\begin{table}[htbp]
\centering
\caption{The five experimental arms. Each arm differs only in the loss function and whether the probabilistic bottleneck is active. All other settings (architecture depth, optimiser, learning rate, dataset, training duration) are identical.}
\label{tab:methods:arms}
\begin{tabular}{p{1.2cm}p{4.5cm}p{2.3cm}p{5cm}}
\toprule
\textbf{Arm} & \textbf{Loss components} & \textbf{VAE bottleneck} & \textbf{Contribution under test} \\
\midrule
...
E & NLL + MS-SSIM + FFL + KL & Yes & Probabilistic bottleneck and epistemic uncertainty \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Foi noise presets used in the experiments. Effective $\sigma$ is computed as
$\sqrt{a \cdot 0.5 + b}$, evaluated at mean image intensity $\bar{y} = 0.5$, providing a
single-number summary comparable to the additive Gaussian noise levels used in natural-image
denoising benchmarks (e.g.\ $\sigma = 15$, 25, 50 in BSD68).}
...
\begin{table}[htbp]
\centering
\caption{Spatial resolution and channel count at each stage of PP-VAE-Hformer. The input
image is compressed by a factor of $8\times$ spatially (256 to 32 pixels) and expanded
$512\times$ in channel depth before the bottleneck. The decoder mirrors this progression.}
...
\subsection{HybridBlock}
\label{sec:methods:architecture:hybridblock}
...
\begin{equation}
\label{eq:methods:composite}
\mathcal{L}_{\mathrm{total}} = \mathcal{L}_{\mathrm{NLL}}
  + \beta_t\,\mathcal{L}_{\mathrm{KL}}
  + \lambda_{\mathrm{SSIM}}\,\mathcal{L}_{\mathrm{SSIM}}
  + \lambda_{\mathrm{FFL}}\,\mathcal{L}_{\mathrm{FFL}}
\end{equation}
with $\lambda_{\mathrm{SSIM}} = 0.5$, $\lambda_{\mathrm{FFL}} = 0.1$, and $\beta_t$ from
Equation~\ref{eq:methods:beta}. Table~\ref{tab:methods:loss_summary} summarises which
loss components are active in each experimental arm.

\begin{table}[htbp]
\centering
\caption{Loss components active in each experimental arm. A tick indicates that the term
contributes a non-zero gradient during training. The KL term requires the VAE bottleneck
to be active.}
...
\begin{table}[htbp]
\centering
\caption{Summary of evaluation metrics by category. Primary metric is FSIM; all others are
secondary or supplementary. Calibration metrics (ECE) apply only to Arms B--E. Epistemic
ECE applies only to Arm E.}
