...
Figure~\ref{fig:results:training} shows the validation PSNR trajectory and KL divergence schedule across all ablation arms over 200 training epochs. All five arms converged smoothly without divergence events. Validation PSNR was computed every five epochs using a deterministic forward pass; the interleaved \texttt{nan} entries in the training logs between evaluation epochs are by design and do not indicate instability. The KL divergence panel confirms successful $\beta$-annealing for Arm~E: the weighted KL loss grows monotonically from zero to its plateau following the 20-epoch warmup, without evidence of posterior collapse.
...
\midrule
A — Hformer + $\mathcal{L}_{\mathrm{MSE}}$ & [FILL] & [FILL] & [FILL] \\
B — Hformer + NLL                           & [FILL] & [FILL] & [FILL] \\
C — Hformer + NLL + SSIM                    & [FILL] & [FILL] & [FILL] \\
D — Hformer + NLL + SSIM + FFL             & [FILL] & [FILL] & [FILL] \\
E — PP-VAE-Hformer (full)                   & [FILL] & [FILL] & [FILL] \\
DnCNN (Zhang et al., 2017)                 & [FILL] & [FILL] & [FILL] \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
A — Hformer + $\mathcal{L}_{\mathrm{MSE}}$  & ---         & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
B — Hformer + NLL                            & ---         & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
C — Hformer + NLL + SSIM                     & ---         & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
D — Hformer + NLL + SSIM + FFL              & ---         & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
E — PP-VAE-Hformer\dag                       & \checkmark  & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
\midrule
DnCNN (Zhang et al., 2017) \cite{zhang_beyond_2017} & --- & [FILL]\,$\pm$\,[SD] {[CI]} & [FILL]\,$\pm$\,[SD] \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
A one-way ANOVA on PSNR at mid noise across all ablation arms yielded $F([k{-}1],\,[N{-}k]) = \text{[FILL]},\;p \text{ [FILL]}$ ($n=624$ per arm), confirming that at least one arm differs significantly from the others. Post-hoc pairwise comparisons with Bonferroni correction ([FILL] comparisons; adjusted $\alpha = [FILL]$) are reported in Table~\ref{tab:results:stats:pairwise}. Cohen's $d$ is the pooled effect-size estimator; values $>0.8$ indicate a large practical difference \cite{cohen_statistical_1988}.
...
\caption[Qualitative reconstruction -- Pneumonia cases]{
  Qualitative reconstruction examples for three representative Pneumonia test cases at mid noise level using Arm~E (PP-VAE-Hformer).
  \textit{Left to right:} noisy input, clean target, denoised mean ($\hat{\mu}$), aleatoric uncertainty ($\hat{\sigma}_a$, inferno colourmap), absolute reconstruction error ($|\hat{\mu}-y|$, RdBu colourmap), epistemic uncertainty ($\hat{\sigma}_e$, plasma colourmap).
  Elevated aleatoric uncertainty at consolidated lung regions (bright areas in Column~4) is consistent with genuine image ambiguity at pathological sites.}
...
\caption[Qualitative reconstruction -- Normal cases]{
  Qualitative reconstruction examples for three representative Normal test cases at mid noise level using Arm~E (PP-VAE-Hformer). Layout identical to Figure~\ref{fig:results:qual:pneumonia}. In Normal cases, aleatoric uncertainty concentrates at anatomical boundaries (cardiac silhouette, diaphragm) rather than being diffusely elevated across the lung field, in contrast to the Pneumonia cases in Figure~\ref{fig:results:qual:pneumonia}.}
...
Figure~\ref{fig:results:dosecurve} shows PSNR and SSIM as a function of noise severity for all arms. All arms degrade monotonically from low to high noise, as expected. The relative ordering of arms is [preserved / reversed / changes at high noise — FILL after evaluation], indicating that the loss function choice has [similar / different] effects across dose levels.
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$     & [F] $\pm$ [SD] & [F] & 34.623\,$\pm$\,[SD] & 0.9269 & [F] & [F] \\
B — NLL                               & [F] $\pm$ [SD] & [F] & 33.440\,$\pm$\,[SD] & 0.9091 & [F] & [F] \\
C — NLL + SSIM                        & [F] $\pm$ [SD] & [F] & 32.357\,$\pm$\,[SD] & 0.8868 & [F] & [F] \\
D — NLL + SSIM + FFL                 & [F] $\pm$ [SD] & [F] & 33.435\,$\pm$\,[SD] & 0.9089 & [F] & [F] \\
E — PP-VAE (full)                     & [F] $\pm$ [SD] & [F] & [F]\,$\pm$\,[SD]    & [F]    & [F] & [F] \\
\midrule
DnCNN \cite{zhang_beyond_2017}        & [F] $\pm$ [SD] & [F] & [F]\,$\pm$\,[SD]    & [F]    & [F] & [F] \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:results:subgroup} stratifies reconstruction performance by diagnostic class. The Kermany test set comprises [234] Normal and [390] Pneumonia cases (ratio 38:62). If the PP-VAE framework preserves pathological features differentially, we would expect Arm~E to show a smaller performance gap between classes compared to the L$_2$ baseline (Arm~A).
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
B — NLL                            & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
C — NLL + SSIM                     & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
D — NLL + SSIM + FFL              & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
E — PP-VAE                         & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
DnCNN \cite{zhang_beyond_2017}     & [F]\,$\pm$\,[SD] & [F]\,$\pm$\,[SD] & [F] dB & [***] \\
\bottomrule
\end{tabular}
\end{table}
