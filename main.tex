...
\caption[Latent space projections -- Arm H (PP-VAE, cyc+fb KL)]{%
  ...
  The cyclical KL + free-bits schedule produces wider latent dispersion than Arms~E, G, and~P,
  but ARI ($0.187$) is lower than Arm~F ($0.310$): the free-bits KL floor clips the KL penalty,
  partially suppressing the periodic latent-code release that drives Arm~F's superior class
  separation. Class clusters remain overlapping, as expected under KL regularisation.}
...
\midrule
E & Linear (collapsed)       & $0.024$ & $0.008$ & Near-posterior-collapse; uninformative latent \\
F & Cyclical only             & $\mathbf{0.310}$ & $\mathbf{0.105}$ & Best class separation; periodic KL release \\
G & Free bits only            & $0.045$ & $0.036$ & Modest recovery; KL floor compresses code \\
H & Cyclical + free bits      & $0.187$ & $0.064$ & Good separation; free bits partially offsets cyc benefit \\
P & Cyc+fb + PReLU (full)     & $0.031$ & $0.048$ & Near-collapse; PReLU instability dominates \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item Computed from 448 test images (150 Normal / 149 Bacterial / 149 Viral subsampled).
  $k$-means ARI: $k=3$, random\_state=42. Latent vectors: global average pool of
  $\mu_z \in \mathbb{R}^{64 \times 32 \times 32} \to \mathbb{R}^{64}$.
\end{tablenotes}
\end{table}

Table~\ref{tab:results:latent:quant} reveals a striking and unexpected finding: Arm~F (cyclical KL \emph{only}) achieves by far the highest ARI ($0.310$) and Silhouette Score ($0.105$) of all five VAE arms, substantially exceeding Arm~H (cyclical + free-bits, ARI~$0.187$) despite Arm~H achieving higher reconstruction PSNR ($32.762$ vs.\ $32.734$~dB). This dissociation between reconstruction quality and latent class separation is a central result of the latent space analysis.
...
Second, Arm~F (cyclical KL only, ARI~$=0.310$) achieves substantially higher class separation than all other arms, including Arm~H (ARI~$=0.187$). This is \emph{not} the arm with the best reconstruction quality (Arm~H outperforms F by $0.028$~dB), revealing a reconstruction-discrimination trade-off: the free-bits mechanism that improves Arm~H's reconstruction stability suppresses the latent class information that Arm~F retains. If downstream tasks require class-separable latent codes (e.g.\ for retrieval or anomaly detection), Arm~F's schedule is preferable; if reconstruction quality is the primary objective, Arm~H's combined schedule is preferable.

Third, Arm~E (linear KL, ARI~$=0.024$) shows near-collapse confirmed by flat projections and near-zero ARI. Linear annealing is insufficient for this dataset and architecture.

Fourth, Arm~P (PReLU full model, ARI~$=0.031$) also shows near-collapse despite using cyclical+free-bits scheduling, confirming that the PReLU instability documented in the reconstruction analysis (Section~\ref{sec:results:error}) corrupts latent space structure in addition to degrading reconstruction quality. PReLU therefore fails along both the reconstruction and latent structure axes simultaneously.
