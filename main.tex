...
\end{table}
Four sequential phases were executed: (1) dataset preparation and noise simulation; (2) training across all 16 arms and 5 retrained baselines; (3) quantitative evaluation on the held-out test set; and (4) calibration analysis of uncertainty estimates from NLL-trained arms. All phases used PyTorch \cite{paszke_pytorch_2019} (\texttt{scripts/train\_proposed.py}).
...
With the experimental apparatus fully specified, Chapter~\ref{chap:results} presents the outcomes: reconstruction performance across all 19 arms and five retrained baselines, latent space geometry, uncertainty calibration, and subgroup behaviour across Normal, Bacterial Pneumonia, and Viral Pneumonia cases.
...
\caption[Primary reconstruction results at mid noise level]{%
  Test-set reconstruction performance at \textit{mid} noise level ($\eta=200$, $n=624$) for all 16 ablation arms, 4 retrained KAIR baselines, DnCNN, BM3D (classical oracle-$\sigma$ baseline), and Palette-DDPM (conditional diffusion, 200 epochs).
...
\begin{enumerate}
\item \textbf{Reconstruction (RO1).} Arm~A leads PSNR ($34.62$~dB); Arm~R leads perceptual quality (FSIM~$0.9332$, LPIPS~$0.2192$); Arm~D achieves the best uncertainty-reconstruction trade-off ($33.44$~dB, calibrated NLL~$-2.67$).
...
\item \textbf{Baselines (RO7).} All five retrained baselines ($31.18$--$32.80$~dB) are outperformed by Arm~A ($+1.82$~dB above DnCNN); SwinIR trails all CNN baselines.
...
Fair comparisons use the same Kermany corpus and Poisson-Gaussian noise model. BM3D with oracle $\sigma$ achieves $30.716$~dB, trailing every learned arm by $\geq 1.9$~dB, confirming that signal-dependent heteroscedastic noise falls outside AWGN assumptions. DnCNN retrained achieves $32.798$~dB; NLL arms B and D exceed this by $+0.66$~dB and $+0.64$~dB respectively ($d\approx0.65$) while simultaneously providing calibrated per-pixel uncertainty maps. Arm~A ($34.618$~dB) exceeds DnCNN by $+1.82$~dB attributable to Hformer's attention-based long-range modelling. DDPM ($13.52$~dB) is an honest negative finding: 200 epochs is insufficient for Palette-style diffusion to converge; the trajectory was still monotonically rising at epoch~200, demonstrating that discriminative architectures are far more budget-efficient.
...
This dissertation designed, implemented, and evaluated PP-VAE-Hformer, a probabilistic denoising framework for low-dose paediatric CXR. The system combines a hybrid CNN-Transformer (Hformer) encoder-decoder with an optional VAE bottleneck and a four-term composite loss (NLL, MS-SSIM, FFL, KL). A 16-arm ablation study on the 624-image Kermany test set, with 210 Bonferroni-corrected pairwise comparisons against five retrained baselines, BM3D, and DDPM, establishes which loss terms contribute to pixel fidelity, perceptual quality, and calibrated uncertainty quantification.
