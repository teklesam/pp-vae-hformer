...
Denoising methods fall into four families, each with a characteristic failure mode in medical imaging \cite{willemink_preparing_2020}. Classical filters (BM3D, NLM, TV) lack semantic intelligence and over-smooth fine pathology. Discriminative CNNs and hybrid Transformer architectures \cite{zhang_beyond_2017, zhang_drunet_2021, liang_swinir_2021, zhang_hformer_2023} learn efficiently but rely on pixel-wise losses that blur fine structural detail and provide no uncertainty output. Generative adversarial methods \cite{goodfellow_generative_2014} synthesise sharp textures but hallucinate anatomically plausible yet clinically false structures. Diffusion models \cite{ho_denoising_2020} achieve high perceptual quality but erase pathology under prior dominance and require hundreds of inference steps.
...
The Variational Autoencoder (VAE) \cite{kingma_auto-encoding_2014} offers a probabilistic alternative.\footnote{A \emph{Variational Autoencoder} (VAE) \cite{kingma_auto-encoding_2014} consists of two jointly trained neural networks. The \emph{encoder} $q_\phi(z|x)$ compresses an input image $x$ into a compact \emph{latent code} $z$, but rather than outputting a single point it outputs a distribution: a mean vector $\mu_\phi(x)$ and a variance vector $\sigma^2_\phi(x)$. A sample $z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon$ (where $\epsilon \sim \mathcal{N}(0,I)$) is drawn via the \emph{reparameterisation trick}, which allows gradients to flow through the stochastic sampling step. The \emph{decoder} $p_\theta(x|z)$ reconstructs the image from this sample. The \emph{latent space} is the lower-dimensional vector space in which all $z$ values reside; geometrically similar images map to nearby latent codes, giving the space smooth interpolation properties \cite{kingma_auto-encoding_2014}.} The encoder maps input $x$ to a posterior $q_\phi(z|x)$\footnote{The \emph{posterior} $q_\phi(z|x)$ is the distribution over latent codes $z$ given the observed input $x$. The encoder parameterises it as a Gaussian $\mathcal{N}(\mu_\phi(x),\,\sigma^2_\phi(x)I)$. The \emph{latent space} is the continuous lower-dimensional manifold in which these codes reside; the prior $p(z) = \mathcal{N}(0,I)$ encourages codes to be regularly arranged, so that random draws from the prior decode into plausible images.} in a latent space, enabling pixel-wise aleatoric and epistemic uncertainty maps\footnote{\emph{Aleatoric uncertainty} is irreducible: it reflects genuine randomness in the data itself, here the quantum shot noise of the X-ray photon flux, which no amount of additional training data or model capacity can eliminate. \emph{Epistemic uncertainty} is reducible: it reflects the model's own ignorance in regions where the training data provide little constraint, and in principle decreases as the model sees more data or is better regularised. The two types are measured differently: aleatoric uncertainty is captured by a learned per-pixel variance head $\hat{\sigma}^2_a$ on the decoder; epistemic uncertainty is estimated by running the stochastic encoder-decoder $K=20$ times and measuring the pixel-wise variance across reconstructions \cite{kendall_what_2017}.} via repeated sampling. However, the standard ELBO\footnote{The \emph{Evidence Lower BOund} (ELBO) is the VAE training objective. It lower-bounds the marginal log-likelihood $\log p(x)$ of the data and decomposes as $\mathrm{ELBO} = \mathbb{E}_{q_\phi(z|x)}[\log p_\theta(x|z)] - D_{\mathrm{KL}}(q_\phi(z|x)\,\|\,p(z))$. The first term rewards faithful reconstruction; the second, the KL divergence, penalises the encoder for producing a posterior that deviates from the unit Gaussian prior $p(z) = \mathcal{N}(0,I)$. When the decoder uses a fixed-variance isotropic Gaussian likelihood, maximising the reconstruction term is mathematically equivalent to minimising unweighted pixel-wise MSE, which is why standard VAEs blur high-frequency detail \cite{bredell_explicitly_2023}.}'s factorised Gaussian likelihood reduces to unweighted MSE, producing blur, and the KL penalty can force the posterior to collapse to the prior,\footnote{\emph{Posterior collapse} is a pathological failure mode in which the KL penalty overwhelms the reconstruction signal during training, causing the encoder's posterior $q_\phi(z|x)$ to converge to the prior $\mathcal{N}(0,I)$ regardless of what the input image looks like. Once collapsed, the latent code $z$ carries no image-specific information: the decoder ignores it entirely and outputs the same average image for every patient. Epistemic uncertainty maps become a spatially flat field, losing all diagnostic structure. Posterior collapse is most likely when the KL weight is large early in training, before a useful reconstruction signal is established \cite{lucas_dont_2019}.} causing the model to output a generic average image regardless of patient anatomy \cite{bredell_explicitly_2023, zhao_loss_2017, lucas_dont_2019}.
...
The WindowAttnBlock applies multi-head self-attention\footnote{Self-attention \cite{vaswani_attention_2017} computes, for each position in a feature map, a weighted sum of all other positions' values, where the weights (attention scores) measure pairwise similarity between a query vector at the current position and key vectors at all other positions. This allows the network to capture long-range dependencies regardless of spatial distance, something that fixed-kernel convolutions cannot do. In image processing, full global self-attention over a $32\times32$ feature map requires computing $(32\times32)^2 = 1,048,576$ pairwise scores. Window-restricted attention (as in the Swin Transformer \cite{liu_swin_2021}) limits each query to attend only within a small non-overlapping local window, reducing the per-window score count to $(8\times8)^2 = 4,096$ at a fraction of the global attention cost.} within non-overlapping $8\times8$ windows \cite{liu_swin_2021}, reducing the $4.3\times10^9$ pairwise scores of global attention to 4,096 per window while retaining sufficient context for the network to understand what an anatomical region should look like given its neighbours. The block uses pre-layer normalisation and a two-layer feedforward network with GELU activation. Two HybridBlocks are stacked at each stage.
