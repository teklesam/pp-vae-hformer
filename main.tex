...
\begin{tikzpicture}[
  >=Stealth,
  every node/.style={font=\small},
  imgbox/.style={draw=gray!60, fill=gray!10, rounded corners=3pt,
                 minimum width=1.4cm, minimum height=1.4cm, align=center},
  procbox/.style={draw=blue!50!black, fill=blue!7, rounded corners=5pt,
                  minimum width=2.6cm, minimum height=1.1cm, align=center,
                  inner sep=5pt},
  noisebox/.style={draw=orange!70!black, fill=orange!7, rounded corners=5pt,
                   minimum width=2.2cm, minimum height=1.1cm, align=center,
                   inner sep=5pt},
  armbox/.style={draw=gray!50, fill=white, rounded corners=3pt,
                 minimum width=2.5cm, minimum height=0.55cm, align=center,
                 inner sep=3pt, font=\footnotesize},
  outbox/.style={draw=teal!60!black, fill=teal!8, rounded corners=5pt,
                 minimum width=2.6cm, minimum height=0.75cm, align=center,
                 inner sep=4pt, font=\footnotesize},
  arr/.style={->, thick, gray!70!black},
  darr/.style={->, thick, teal!70!black},
]

%% ── Column positions ────────────────────────────────────────────
\def\xA{0}\def\xB{2.3}\def\xC{5.0}\def\xD{8.3}\def\xE{11.8}

%% ── Input image (placeholder box) ──────────────────────────────
\node[imgbox] (cxr) at (\xA, 0) {\footnotesize CXR\\[-2pt]\footnotesize (clean)};
\node[above=0.05cm of cxr, font=\footnotesize\itshape] {Kermany dataset};

%% ── Noise simulation ────────────────────────────────────────────
\node[noisebox] (noise) at (\xB, 0) {Poisson-Gaussian\\[-2pt]corruption\\[-2pt]$\eta \in \{50,100,200,300\}$};

%% ── PP-VAE-Hformer (model) ──────────────────────────────────────
\node[procbox] (model) at (\xC+1, 0) {\textbf{PP-VAE-Hformer}\\[-2pt]Encoder--Bottleneck--Decoder\\[-2pt](shared architecture)};

%% ── Ablation arms panel ─────────────────────────────────────────
\node[armbox, fill=blue!5,  draw=blue!40]   (armA) at (\xC+1,  1.65) {Arm A: $\mathcal{L}_2$};
\node[armbox, fill=orange!5,draw=orange!40] (armB) at (\xC+1,  1.05) {Arm B: NLL};
\node[armbox, fill=green!5, draw=green!40]  (armC) at (\xC+1,  0.45) {Arm C: NLL + SSIM};
\node[armbox, fill=red!5,   draw=red!40]    (armD) at (\xC+1, -0.45) {Arm D: NLL + SSIM + FFL};
\node[armbox, fill=purple!5,draw=purple!40,very thick] (armE) at (\xC+1, -1.05)
      {Arm E: NLL + SSIM + FFL + KL \textbf{(VAE)}};

\node[above=0.08cm of armA, font=\footnotesize\bfseries] {Loss function variant};

%% ── Outputs ─────────────────────────────────────────────────────
\node[outbox] (out1) at (\xE, 0.75)
      {$\hat{\mu}_y$ \;Denoised image};
\node[outbox] (out2) at (\xE, 0)
      {$\log\hat{\sigma}^2_a$ \;Aleatoric map\\[-1pt](Arms B--E)};
\node[outbox, draw=purple!60!black, fill=purple!6] (out3) at (\xE, -0.75)
      {$\hat{\sigma}^2_e$ \;Epistemic map\\[-1pt](Arm E, MC sampling)};

%% ── Arrows ──────────────────────────────────────────────────────
\draw[arr] (cxr.east)   -- (noise.west);
\draw[arr] (noise.east) -- (model.west);
\draw[darr] (model.east) -- (out1.west);
\draw[darr] (model.east) -- (out2.west);
\draw[darr] (model.east) -- (out3.west);

%% ── Brace spanning ablation arms ────────────────────────────────
\draw[decorate, decoration={brace, amplitude=5pt, mirror},
      gray!60, thick]
  ([xshift=-1.4cm]armA.north west) -- ([xshift=-1.4cm]armE.south west)
  node[midway, left=0.2cm, font=\footnotesize, align=right, text=gray!70!black]
       {5 ablation\\[-2pt]arms};

\end{tikzpicture}
\caption[Experimental pipeline overview]{%
  Overview of the experimental pipeline.
  Clean paediatric chest radiographs from the Kermany dataset are corrupted by
  simulated Poisson-Gaussian noise at four levels ($\eta \in \{50,100,200,300\}$).
  The degraded images are processed by PP-VAE-Hformer, which is trained under five
  ablation arms that differ only in loss function and the presence of the VAE
  probabilistic bottleneck (Arm~E). All arms share the same encoder--decoder
  architecture and training schedule. The model produces a denoised image
  $\hat{\mu}_y$ (all arms), a per-pixel aleatoric uncertainty map
  $\log\hat{\sigma}^2_a$ (Arms~B--E), and an epistemic uncertainty map
  $\hat{\sigma}^2_e$ obtained by Monte Carlo sampling through the VAE bottleneck
  (Arm~E only).}
...
\begin{tikzpicture}[
  ...
  %% Block styles
  imgnode/.style={draw=gray!60, fill=gray!12, rounded corners=3pt,
                  minimum width=1.0cm, minimum height=1.8cm, align=center},
  stemnode/.style={draw=blue!50!black, fill=blue!8, rounded corners=4pt,
                   minimum width=1.5cm, minimum height=0.7cm, align=center, inner sep=3pt},
  encnode/.style={draw=blue!60!black, fill=blue!12, rounded corners=4pt,
                  minimum width=1.6cm, minimum height=1.4cm, align=center, inner sep=4pt},
  downnode/.style={draw=blue!40, fill=blue!5, rounded corners=3pt,
                   minimum width=0.55cm, minimum height=0.55cm, align=center},
  botnode/.style={draw=teal!60!black, fill=teal!10, rounded corners=4pt,
                  minimum width=1.7cm, minimum height=1.4cm, align=center, inner sep=4pt,
                  very thick},
  vaenode/.style={draw=purple!60!black, fill=purple!8, rounded corners=4pt,
                  minimum width=1.6cm, minimum height=1.5cm, align=center, inner sep=4pt,
                  dashed, thick},
  decnode/.style={draw=orange!60!black, fill=orange!8, rounded corners=4pt,
                  minimum width=1.6cm, minimum height=1.4cm, align=center, inner sep=4pt},
  upnode/.style={draw=orange!40, fill=orange!5, rounded corners=3pt,
                 minimum width=0.55cm, minimum height=0.55cm, align=center},
  outnode/.style={draw=teal!60!black, fill=teal!8, rounded corners=4pt,
                  minimum width=1.5cm, minimum height=0.7cm, align=center, inner sep=3pt},
  %% HybridBlock inset
  hbbox/.style={draw=gray!50, fill=gray!4, rounded corners=4pt,
                minimum width=2.2cm, minimum height=1.0cm, align=center, inner sep=3pt},
  %% Annotations
  dimtag/.style={font=\tiny, text=gray!70!black, align=center},
  arrmain/.style={->, thick, gray!70!black},
  arrskip/.style={->, thick, gray!50, dashed},
  arrvae/.style={->, thick, purple!70!black},
]
...
\def\xIn{0}
\def\xStem{1.5}
\def\xEone{3.4}     % Encoder 1
\def\xEtwo{5.4}     % Encoder 2
\def\xEthr{7.4}     % Encoder 3
\def\xBot{9.5}      % Bottleneck
\def\xVae{11.6}     % VAE (Arm E)
\def\xDthr{13.7}    % Decoder 3
\def\xDtwo{15.7}    % Decoder 2
\def\xDone{17.7}    % Decoder 1
\def\xOut{19.7}     % Output heads

%% ── Input ──────────────────────────────────────────────────────
\node[imgnode] (inp) at (\xIn, 0)
      {$\mathbf{x}$\\[2pt]$1{\times}256$\\${\times}256$};
\node[above=0.6cm of inp, dimtag] {Noisy\\CXR};

%% ── Stem ────────────────────────────────────────────────────────
\node[stemnode] (stem) at (\xStem, 0)
      {Stem\\$3{\times}3$ conv};
\node[below=0.05cm of stem, dimtag] {$64{\times}256{\times}256$};

%% ── Encoder stages ──────────────────────────────────────────────
\node[encnode] (enc1) at (\xEone, 0)
      {\textbf{E1}\\HB$\times$2\\$+$Down};
\node[below=0.05cm of enc1, dimtag] {$128{\times}128{\times}128$};

\node[encnode] (enc2) at (\xEtwo, 0)
      {\textbf{E2}\\HB$\times$2\\$+$Down};
\node[below=0.05cm of enc2, dimtag] {$256{\times}64{\times}64$};

\node[encnode] (enc3) at (\xEthr, 0)
      {\textbf{E3}\\HB$\times$2\\$+$Down};
\node[below=0.05cm of enc3, dimtag] {$512{\times}32{\times}32$};

%% ── Bottleneck ──────────────────────────────────────────────────
\node[botnode] (bot) at (\xBot, 0)
      {\textbf{Bottleneck}\\HB$\times$2};
\node[below=0.05cm of bot, dimtag] {$512{\times}32{\times}32$};

%% ── VAE probabilistic bottleneck (Arm E only) ──────────────────
\node[vaenode] (vae) at (\xVae, 0)
      {\textbf{VAE}\\[2pt]$\mathbf{z}_\mu$, $\mathbf{z}_{\log\sigma^2}$\\[2pt]
       $\mathbf{z} = \mathbf{z}_\mu + \boldsymbol{\varepsilon}\odot e^{\frac{1}{2}\mathbf{z}_{\log\sigma^2}}$\\[2pt]
       (Arm E only)};
\node[below=0.05cm of vae, dimtag] {$512{\times}32{\times}32$};

%% ── Decoder stages ──────────────────────────────────────────────
\node[decnode] (dec3) at (\xDthr, 0)
      {\textbf{D3}\\Up$+$skip\\$+$HB$\times$2};
\node[below=0.05cm of dec3, dimtag] {$256{\times}64{\times}64$};

\node[decnode] (dec2) at (\xDtwo, 0)
      {\textbf{D2}\\Up$+$skip\\$+$HB$\times$2};
\node[below=0.05cm of dec2, dimtag] {$128{\times}128{\times}128$};

\node[decnode] (dec1) at (\xDone, 0)
      {\textbf{D1}\\Up$+$skip\\$+$HB$\times$2};
\node[below=0.05cm of dec1, dimtag] {$64{\times}256{\times}256$};

%% ── Dual output heads ───────────────────────────────────────────
\node[outnode] (outmu) at (\xOut,  0.55)
      {$\hat{\mu}_y$\\Denoised};
\node[outnode, draw=orange!70!black, fill=orange!6]
      (outsig) at (\xOut, -0.55)
      {$\log\hat{\sigma}^2_a$\\Aleatoric};
\node[above=0.5cm of outmu, dimtag] {Dual $3{\times}3$\\output head};

%% ── Main flow arrows ────────────────────────────────────────────
\draw[arrmain] (inp.east)  -- (stem.west);
\draw[arrmain] (stem.east) -- (enc1.west);
\draw[arrmain] (enc1.east) -- (enc2.west);
\draw[arrmain] (enc2.east) -- (enc3.west);
\draw[arrmain] (enc3.east) -- (bot.west);
\draw[arrmain] (bot.east)  -- (vae.west);
\draw[arrmain] (vae.east)  -- (dec3.west);
\draw[arrmain] (dec3.east) -- (dec2.west);
\draw[arrmain] (dec2.east) -- (dec1.west);
\draw[arrmain] (dec1.east) -- (outmu.west);
\draw[arrmain] (dec1.east) -- (outsig.west);

%% ── Skip connections (curved, dashed) ──────────────────────────
\def\skipY{-2.5}   % y-level of skip connection routing
\draw[arrskip]
  (enc1.south) -- ++(0,-0.7) -| node[pos=0.02,dimtag,below]{skip $s_1$} (\xDone, \skipY+0.2) -- (dec1.south);
\draw[arrskip]
  (enc2.south) -- ++(0,-1.1) -| node[pos=0.01,dimtag,below]{skip $s_2$} (\xDtwo, \skipY+0.2) -- (dec2.south);
\draw[arrskip]
  (enc3.south) -- ++(0,-1.5) -| node[pos=0.01,dimtag,below]{skip $s_3$} (\xDthr, \skipY+0.2) -- (dec3.south);

%% ── Arms A–D: bypass the VAE ────────────────────────────────────
\draw[arrmain, gray!40, bend left=30]
  ([yshift=4pt]bot.north east) to
  node[above, dimtag, text=gray!50]{Arms A--D (det.)}
  ([yshift=4pt]dec3.north west);

%% ════════════════════════════════════════════════════════════════
%% HYBRIDBLOCK INSET (bottom-right callout)
%% ════════════════════════════════════════════════════════════════
\begin{scope}[shift={(\xEone-0.3,-4.5)}]
  \node[hbbox, minimum width=5.5cm, minimum height=3.2cm] (hbframe) at (2.0,0) {};
  \node[above=0.0cm of hbframe, font=\scriptsize\bfseries, text=gray!80] {HybridBlock (HB)};

  \node[stemnode, minimum width=1.6cm] (rb)   at (0.4, 0.3)  {ResBlock\\$3{\times}3$ GN+GELU};
  \node[stemnode, minimum width=1.6cm,
        draw=purple!50!black, fill=purple!6]
        (wa)   at (3.5, 0.3) {Window\\[-1pt]AttnBlock\\$8{\times}8$ window};
  \node[stemnode, minimum width=0.6cm, draw=gray!50, fill=gray!8]
        (add)  at (2.0, -0.8) {$+$};

  \draw[arrmain, ->] (rb.east)   -- (wa.west);
  \draw[arrmain, ->] (wa.south)  |- (add.east);
  \draw[arrmain, ->] (rb.south)  |- (add.west);
  \node[left=0.0cm of rb, dimtag] {$\mathbf{f}_{in}$};
  \node[below=0.05cm of add, dimtag] {$\mathbf{f}_{out}$};

  % Callout line to Encoder 1
  \draw[gray!40, dashed, thin]
    (hbframe.north) -- ([yshift=-1.4cm]enc1.south);
\end{scope}
...
Figure~\ref{fig:results:training} shows the validation PSNR trajectory for all five ablation arms and the DnCNN baseline over training, together with the $\beta$-annealed KL divergence for Arm~E. All arms converged smoothly without divergence events. Validation PSNR was computed every five epochs using a deterministic forward pass; the interleaved \texttt{nan} entries in the training logs between evaluation epochs are by design and do not indicate instability. Arm~E's KL divergence rises from zero (fully annealed-off at epoch 0) to its plateau value, demonstrating successful KL warmup and the absence of posterior collapse.

\begin{figure}[H]
\centering
\evalfig{figures/evaluation/training_curves}
\caption[Training convergence curves for all ablation arms]{
  \textit{Left:} Validation PSNR (dB) during training across all five ablation arms and the DnCNN baseline, evaluated every five epochs on the held-out validation set.
  \textit{Right:} Weighted KL divergence ($\beta \cdot \mathcal{L}_{\mathrm{KL}}$) as a function of training epoch for Arm~E, demonstrating the $\beta$-annealing schedule from 0 to $\beta_{\max} = 10^{-3}$ over 20 epochs and convergence at the plateau. Arms~A--D are omitted from the right panel as they do not include a KL term.}
...
\begin{table}[htbp]
\centering
\caption[Final training metrics per ablation arm]{Best validation PSNR and SSIM achieved during training for all ablation arms, together with the epoch at which the best checkpoint was saved (used for all test-set evaluations).}
\label{tab:results:training}
\begin{tabular}{lccc}
\toprule
\textbf{Arm} & \textbf{Best val PSNR (dB)} & \textbf{Best val SSIM} & \textbf{Best epoch} \\
\midrule
A — Hformer + $\mathcal{L}_{\mathrm{MSE}}$ & 34.490 & 0.9137 & 199 \\
B — Hformer + NLL                           & 33.280 & 0.8948 & 190 \\
C — Hformer + NLL + SSIM                    & 32.137 & 0.8711 & 199 \\
D — Hformer + NLL + SSIM + FFL             & 33.264 & 0.8945 & 190 \\
E — PP-VAE-Hformer (full)                   & 31.432 & 0.8568 & 50 \\
...
Table~\ref{tab:results:recon:mid} reports final reconstruction performance on the 624-image held-out test set at mid noise level ($\eta = 200$, $n_{\mathrm{Normal}}=234$, $n_{\mathrm{Pneumonia}}=390$). Four metrics are reported: PSNR (pixel fidelity), SSIM (structural similarity), MS-SSIM (multi-scale structural similarity; Wang et al.\ 2003), and LPIPS (learned perceptual image patch similarity via AlexNet; Zhang et al.\ 2018; lower is better). Mean and SD are computed over all 624 test images; 95\% CIs are from 1,000 bootstrap resamples. All arms except E are deterministic at inference; Arm~E uses a stochastic VAE bottleneck with $\mathbf{z} = \boldsymbol{\mu}_z$ at inference time (deterministic decode), with stochastic sampling used only for epistemic uncertainty estimation.

\begin{sidewaystable}[p]
\centering
\caption[Primary reconstruction results at mid noise level]{
  Test-set reconstruction performance at \textit{mid} noise level ($\eta=200$, $n=624$).
  Values are mean\,$\pm$\,SD with 95\% bootstrap CI in brackets.
  \textbf{Bold} = best per metric column.
  LPIPS $\downarrow$ = lower is better.
  All results are final (fully trained checkpoints).}
\label{tab:results:recon:mid}
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
\textbf{Method} & \textbf{VAE} & \textbf{PSNR (dB)} & \textbf{SSIM} & \textbf{MS-SSIM} & \textbf{LPIPS $\downarrow$} \\
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & ---        & $\mathbf{34.623 \pm 1.079}$ {[34.54, 34.71]} & $\mathbf{0.9137 \pm 0.0139}$ {[0.913, 0.915]} & $\mathbf{0.9731}$ & $\mathbf{0.1289}$ \\
B — NLL                            & ---        & $33.457 \pm 1.031$ {[33.37, 33.54]}          & $0.8948 \pm 0.0153$ {[0.894, 0.896]}          & $0.9645$          & $0.1594$ \\
C — NLL + SSIM                     & ---        & $32.347 \pm 1.018$ {[32.27, 32.43]}          & $0.8711 \pm 0.0174$ {[0.870, 0.872]}          & $0.9523$          & $0.1857$ \\
D — NLL + SSIM + FFL              & ---        & $33.437 \pm 1.018$ {[33.35, 33.52]}          & $0.8945 \pm 0.0152$ {[0.893, 0.896]}          & $0.9644$          & $0.1583$ \\
E — PP-VAE-Hformer                 & \checkmark & $31.626 \pm 0.950$ {[31.55, 31.70]}          & $0.8568 \pm 0.0197$                           & $0.9470$          & $0.2032$ \\
\midrule
...
\begin{tablenotes}
\footnotesize
\item PSNR and SSIM: higher is better. LPIPS: lower is better (AlexNet backbone, Zhang et al.\ \citeyear{zhang_unreasonable_2018}). LPIPS not reported for Arm~A and DnCNN as the evaluation script computes it only for NLL-trained arms.
\item Arm~E is the only model producing calibrated aleatoric and epistemic uncertainty maps (Section~\ref{sec:results:calibration}).
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
...
A one-way ANOVA on PSNR at mid noise across all six arms yielded $F(5,\,3738) = 663.47,\;p \approx 0$, confirming that loss-function choice produces highly significant differences in reconstruction quality. Post-hoc pairwise comparisons with Bonferroni correction (15~comparisons; adjusted $\alpha = 0.0033$) are reported in Table~\ref{tab:results:stats:pairwise}. Cohen's $d$ is the pooled effect-size estimator; values $|d| > 0.8$ indicate a large practical difference \cite{cohen_statistical_1988}.

Four findings emerge from the final pairwise comparisons. First, Arms~B and~D are statistically indistinguishable ($p_{\mathrm{Bonf}} = 1.000$, $d = +0.02$), confirming that FFL precisely cancels the PSNR cost introduced by the SSIM term without adding further benefit on pixel metrics.\footnote{This equivalence ($d = +0.02$, effectively zero) is consistent with the spectral-bias account: the SSIM term suppresses high-frequency gradients (Arm~C, $-1.07$~dB vs Arm~B) and FFL exactly restores them, leaving the net effect zero. It does not imply that FFL is uninformative — Arm~D achieves a marginally lower LPIPS ($0.1583$ vs $0.1594$), suggesting a perceptual benefit invisible to PSNR.} Second, Arms~B and~D both outperform the fully-retrained DnCNN baseline ($d \approx +0.65$--$0.67$, $p < 0.001$), demonstrating that NLL-based probabilistic training improves pixel-level reconstruction over the MSE-trained deterministic baseline while additionally providing calibrated uncertainty maps unavailable from DnCNN.\footnote{This addresses Research Gap~3 identified in Section~\ref{sec:lit:architectures:plateau}: that deterministic MSE baselines cannot provide uncertainty information. Arms~B and~D simultaneously close the performance gap and add a strictly new output modality.} Third, Arm~C (NLL + SSIM without FFL) is outperformed by DnCNN ($d = -0.46$, $p < 0.001$), showing that SSIM regularisation without frequency-domain compensation is actively harmful.\footnote{The effect size strengthened from the preliminary evaluation ($d = -0.34$) once DnCNN completed full training: the final DnCNN ($32.798$~dB) is stronger than the epoch-120 preliminary ($32.694$~dB), making the disadvantage of Arm~C more apparent.} Fourth, Arm~E (full PP-VAE, epoch~50 best) operates $1.17$~dB below Arms~B/D ($d = -1.83$ to $-1.85$), consistent with the KL-divergence bottleneck imposing a regularisation cost on reconstruction fidelity in exchange for a structured, sampleable latent space — a known trade-off in variational inference \cite{bowman_generating_2016}.

\begin{table}[htbp]
\centering
\caption[Bonferroni-corrected pairwise comparisons of PSNR]{
  All 15 pairwise $t$-tests (Bonferroni-corrected, $n_{\mathrm{comp}}=15$) for PSNR at mid noise level ($n=624$ per arm).
  Positive $d$ = Arm~1 $>$ Arm~2. *** $p<0.001$, n.s.\ not significant. All results final.}
\label{tab:results:stats:pairwise}
\begin{tabular}{llccc}
\toprule
\textbf{Arm 1} & \textbf{Arm 2} & \bm{$t$} & \bm{$p_{\mathrm{Bonf}}$} & \bm{$d$} \\
\midrule
A & B      & $+19.6$ & $2.08 \times 10^{-73}$  *** & $+1.11$ \\
A & C      & $+38.5$ & $1.38 \times 10^{-213}$ *** & $+2.18$ \\
A & D      & $+20.0$ & $9.20 \times 10^{-76}$  *** & $+1.13$ \\
A & E      & $+52.1$ & $< 10^{-300}$           *** & $+2.95$ \\
A & DnCNN  & $+31.8$ & $2.38 \times 10^{-162}$ *** & $+1.80$ \\
B & C      & $+19.2$ & $1.66 \times 10^{-71}$  *** & $+1.09$ \\
B & D      & $+0.4$  & $1.000$                 n.s. & $+0.02$ \\
B & E      & $+32.7$ & $1.82 \times 10^{-168}$ *** & $+1.85$ \\
B & DnCNN  & $+11.8$ & $1.95 \times 10^{-29}$  *** & $+0.67$ \\
C & D      & $-18.9$ & $1.68 \times 10^{-69}$  *** & $-1.07$ \\
C & E      & $+13.1$ & $3.78 \times 10^{-35}$  *** & $+0.74$ \\
C & DnCNN  & $-8.1$  & $1.07 \times 10^{-14}$  *** & $-0.46$ \\
D & E      & $+32.3$ & $3.39 \times 10^{-166}$ *** & $+1.83$ \\
D & DnCNN  & $+11.5$ & $6.64 \times 10^{-28}$  *** & $+0.65$ \\
E & DnCNN  & $-21.9$ & $6.72 \times 10^{-89}$  *** & $-1.24$ \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:results:dosecurve} shows PSNR and SSIM as a function of noise severity for all six arms and the DnCNN baseline. All arms degrade monotonically from low to high noise, as expected. The relative ordering is preserved across all noise levels (Arm~A $>$ Arm~D $\approx$ Arm~B $>$ DnCNN $>$ Arm~C $>$ Arm~E), indicating that the loss-function ranking is robust to degradation severity. The gap between Arm~C (NLL+SSIM) and the remaining NLL arms widens slightly at high noise ($\eta=300$), suggesting that SSIM-only structural regularisation becomes less effective at suppressing the increased stochastic fluctuations in the Poisson-Gaussian model. Arm~E's performance gap relative to Arm~D is consistent across all three noise levels ($\approx$$1.2$~dB), indicating that the KL bottleneck cost is noise-level-independent.
...
\caption[Ablation study bar chart]{
  Incremental contribution of each loss term at mid noise level (mean\,$\pm$\,SD, $n=624$).
  Blue bars (left axis): PSNR (dB). Coral bars (right axis): SSIM.
  Left to right corresponds to Arms A--E, showing the effect of adding NLL (A$\to$B), SSIM (B$\to$C), FFL (C$\to$D), and the VAE bottleneck + KL (D$\to$E).}
\label{fig:results:ablation}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Full ablation results across all noise levels]{Full ablation results across all three noise presets (mean\,$\pm$\,SD). DnCNN is included as a fair external baseline (retrained on Kermany CXR with identical Poisson-Gaussian noise model). \textbf{Bold} = best per noise level. All results final.}
...
\textbf{Arm} & PSNR & SSIM & PSNR & SSIM & PSNR & SSIM \\
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$     & $\mathbf{36.589 \pm 1.081}$ & $\mathbf{0.9330}$ & $\mathbf{34.623 \pm 1.079}$ & $\mathbf{0.9137}$ & $\mathbf{32.914 \pm 1.067}$ & $\mathbf{0.8939}$ \\
B — NLL                               & $35.584 \pm 1.031$ & $0.9208$ & $33.457 \pm 1.031$ & $0.8948$ & $31.562 \pm 1.031$ & $0.8671$ \\
C — NLL + SSIM                        & $34.571 \pm 0.984$ & $0.9050$ & $32.347 \pm 1.018$ & $0.8711$ & $30.386 \pm 1.018$ & $0.8363$ \\
D — NLL + SSIM + FFL                 & $35.568 \pm 1.022$ & $0.9208$ & $33.437 \pm 1.018$ & $0.8945$ & $31.531 \pm 1.013$ & $0.8667$ \\
E — PP-VAE (full)                     & $33.731 \pm 0.953$ & $0.8921$ & $31.626 \pm 0.950$ & $0.8568$ & $29.687 \pm 0.979$ & $0.8200$ \\
\midrule
