...
A key milestone within the Convolutional Neural Network (CNN) generation is \emph{residual learning} \cite{he_deep_2016}: rather than training a network to directly output a clean image, DnCNN \cite{zhang_beyond_2017} trains a network to predict the \emph{noise} itself and subtract it from the input. Formally, if $z$ is the noisy input and $y$ is the clean target, the network learns a mapping $\mathcal{F}(z)$ such that $\hat{y} = z - \mathcal{F}(z)$. This residual formulation is equivalent to inserting an identity skip connection from input to output, so that the network only needs to learn what to \emph{remove} rather than what a clean image looks like. Residual learning remains the structural principle in every descendant architecture, including Hformer and PP-VAE-Hformer.

A common misconception is to treat the Variational Autoencoder (VAE) as a competing denoising method separate from CNNs or Transformers. It is not. The VAE is a \emph{probabilistic bottleneck} that can be inserted into any existing encoder-decoder architecture. PP-VAE-Hformer uses it precisely this way: the Hformer encoder-decoder backbone performs the denoising computation, while the VAE bottleneck adds the ability to quantify how uncertain the network is. The two components are complementary, not competing. The subsections below examine each historical generation in detail.
...
Each prior generation contributes a distinct capability that the next generation builds upon. Critically, PP-VAE-Hformer is not a sixth competing family — it is a deliberate synthesis of the best contribution from every prior approach, assembled to address the specific clinical constraints of paediatric CXR denoising.
...
The proposed framework, PP-VAE-Hformer, directly addresses each of these gaps. In brief, it inserts a Variational Autoencoder probabilistic bottleneck into an Hformer encoder-decoder backbone and trains it with a four-term composite loss combining heteroscedastic negative log-likelihood, structural similarity, focal frequency regularisation, and a KL divergence term. This design is introduced conceptually in Section~\ref{sec:intro:contributions} and specified in full mathematical detail in Chapter~\ref{chap:methods}. Table~\ref{tab:intro:gaps} summarises the gap-to-solution mapping; the primary literature underpinning each gap is reviewed in depth in Chapter~\ref{chap:lit}.
...
The Kermany et al.\ \cite{kermany_identifying_2018} collection comprises 5,856 anterior-posterior chest radiographs from children aged 1--5 years, labelled as normal or pneumonia (bacterial/viral). The full-resolution collection is used at $256\times256$ pixels rather than the $28\times28$ MedMNIST \cite{yang_medmnist_2023} derivative, because diagnostically relevant high-frequency structures (pleural edges, consolidation margins) are absent at low resolution. Dataset partitioning, preprocessing, and training split are detailed in Section~\ref{sec:methods:dataset}.
