...
\begin{titlepage}
  ...
  {\large \today}\\[0.5cm]
  {\large Word Count: $\approx$11{,}250}
...
The total word count of this dissertation is approximately 11{,}250 words, which is within the limit stipulated by the MPhil Population Health Sciences regulations.
...
Kendall and Gal \cite{kendall_what_2017} distinguished aleatoric uncertainty (irreducible data noise, e.g. low photon count) from epistemic uncertainty (model uncertainty, reducible with more data or better specification). For aleatoric uncertainty, the appropriate training loss is the heteroscedastic NLL:
...
A denoised and a hallucinated image can be visually indistinguishable. The Bayesian framework makes the distinction precise: the posterior $p(y \mid z) \propto p(z \mid y)\,p(y)$ is dominated by the likelihood when SNR is high (signal retrieval) and by the prior when SNR is low (prior sampling). Under the Foi model, dark anatomical regions have low SNR, so reconstructions there are necessarily prior-influenced. Mutual information theory places a hard limit: no algorithm can recover information absent from the observation \cite{lucas_dont_2019}. An L2-trained network outputs a single image $\hat{y}$ with no signal about which regime applies.
...
Chapter~\ref{chap:methods} specifies the experimental design that addresses them.
...
\end{equation}
where $a \geq 0$ is the Poisson gain parameter and $b \geq 0$ the Gaussian read-out variance. This affine variance relationship predicts that dark regions (pleural spaces, lung bases) are substantially noisier than bright regions (aerated parenchyma), confirmed with $R^2 > 0.99$ against real CCD sensor data \cite{foi_practical_2008}.

\subsection{Implementation}
\label{sec:methods:noise:implementation}

Noise was generated on-the-fly: a fresh independent sample was drawn for every image in every batch, so the model sees a different noisy realisation on each epoch and generalises across the noise distribution.
...
Unlike additive Gaussian benchmarks (fixed $\sigma$ per image), the Foi model produces spatially heterogeneous noise: dark anatomical regions receive substantially higher variance than bright regions. This spatial heterogeneity is precisely what the heteroscedastic loss (Arms B--E) is designed to model.
...
The proposed model, PP-VAE-Hformer, is a U-Net encoder-decoder whose stages are built from HybridBlocks, a design
...
\paragraph{ResBlock.}
The ResBlock applies two $3 \times 3$ convolutions with GroupNorm and GELU activation, with a residual skip connection \cite{he_deep_2016}.

\paragraph{WindowAttnBlock.}
The WindowAttnBlock applies multi-head self-attention within non-overlapping $8\times8$ windows \cite{liu_swin_2021}, reducing the $4.3\times10^9$ pairwise scores of global attention to 4,096 per window while retaining sufficient context across anatomical boundaries. The block uses pre-normalisation with LayerNorm and a two-layer feedforward network. Two HybridBlocks are stacked at each stage.
...
Arm E adds the VAE bottleneck (Section~\ref{sec:methods:architecture:bottleneck}) and Kullback-Leibler divergence regularisation \cite{kullback_information_1951}. The KL term measures how far the learned posterior
...
To prevent posterior collapse \cite{fu_cyclical_2019, bowman_generating_2016}, the KL weight $\beta$ is annealed from 0 to $\beta_{\max} = 10^{-3}$
...
All arms were trained using AdamW \cite{kingma_adam_2015} with learning rate $2\times10^{-4}$ and weight decay $10^{-4}$.

The learning rate was decayed via cosine annealing from $2\times10^{-4}$ to $10^{-6}$ over 200 epochs (\texttt{CosineAnnealingLR}, $T_{\max}=200$). Training used automatic mixed precision (\texttt{autocast} + \texttt{GradScaler}) with gradient norm clipping at 1.0 to prevent spikes during KL annealing.
...
Metrics were computed on the 624-image held-out test set at each noise preset. Reported values are means over the test set with 95\% bootstrap confidence intervals (1,000 resamples).
...
The EPI measures gradient preservation at anatomical boundaries, computed as SSIM between the Sobel gradient maps of the denoised and clean images. EPI near 1.0 indicates perfect edge preservation; below 0.9 suggests boundary blurring.
...
\endfirsthead
%
\multicolumn{5}{l}{\small\itshape Table~\ref{tab:results:subgroup:combined} \textit{continued from previous page}} \\[3pt]
\toprule
\textbf{Group} & \textbf{Arm / Model} & \textbf{Normal PSNR (dB)} & \textbf{Pneumonia PSNR (dB)} & $\bm{\Delta}$ \\
\midrule
\endhead
...
\multicolumn{5}{p{0.97\linewidth}}{\footnotesize
  $\Delta$ = Pneumonia PSNR $-$ Normal PSNR (dB). The Pneumonia-over-Normal advantage ($\Delta \approx +1.0$~dB) is consistent across all arms and reflects a data-domain property: alveolar consolidation produces stronger local contrast gradients than the clear lung field, providing richer per-pixel reconstruction signal. All pairwise differences are significant at $p<0.001$ (independent-samples $t$-test, Bonferroni-corrected for 21 comparisons).} \\
\endlastfoot
%
