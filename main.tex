...
A key milestone within the CNN generation is \emph{residual learning} \cite{he_deep_2016}: rather than training a network to directly output a clean image, DnCNN \cite{zhang_beyond_2017} trains a network to predict the \emph{noise} itself and subtract it from the input. Formally, if $z$ is the noisy input and $y$ is the clean target, the network learns a mapping $\mathcal{F}(z)$ such that $\hat{y} = z - \mathcal{F}(z)$. This residual formulation is equivalent to inserting an identity skip connection from input to output, so that the network only needs to learn what to \emph{remove} rather than what a clean image looks like. Residual learning remains the structural principle in every descendant architecture, including Hformer and PP-VAE-Hformer.
...
\begin{enumerate}
  ...
  \item \textbf{Pathology Preservation Validation:} Compare PP-VAE-Hformer against five retrained baselines (DnCNN, IRCNN, FFDNet, DRUNet, SwinIR) and two SOTA extensions (NAFNet, SCUNet) on the full Kermany paediatric CXR dataset \cite{kermany_identifying_2018} at $256\times256$ pixels.
...
The LNT model \cite{international_commission_on_radiological_protection_2007_2007, national_research_council_health_2006} predicts linear cancer risk with dose and no safe threshold. Four large cohort studies have replaced inference with direct observation. Mathews et al.\ \cite{mathews_cancer_2013} followed 680,000 Australian children for up to 24 years after CT, finding an incidence rate ratio of 1.24. The EPI-CT consortium (948,174 patients, nine countries) found ERR of 1.27 per 100\,mGy for brain tumours \cite{hauptmann_brain_2023} and 1.96 per 100\,mGy for haematological malignancies with elevated risk detectable at bone marrow doses as low as 10--15\,mGy \cite{bosch_de_basea_gomez_risk_2023}. Smith-Bindman et al.\ \cite{smith-bindman_hematologic_2025} followed 3.7 million children across the USA and Canada; their findings are summarised in Table~\ref{tab:lit:dose}.
...
The aleatoric uncertainty map integrates directly with the ESPR three-tier pathway \cite{lovrenski_esr_2025}: high-$\hat{\sigma}^2$ pixels identify anatomical regions where lung ultrasound (LUS) should be targeted to provide radiation-free confirmation. Calibration of the uncertainty map (via ECE, Chapter~4) is the clinical quality criterion: a poorly calibrated map that assigns low $\hat{\sigma}^2$ to prior-sampled pixels is clinically dangerous, like a mislabelled medication. No comparison method reports calibration.
...
HybridBlocks per stage & 2 & Pilot: 1 block undertrained; 3 exceeded GPU memory \\
...
MC sampling passes ($K$) & 20 & Pilot: $K{=}100$ changed epistemic maps by $<1\%$; \\
...
All experiments used CSD3 NVIDIA A100 80\,GB GPUs (one per arm, SLURM \texttt{ampere} partition). Code: Python 3.12 / PyTorch 2.5.1 / CUDA 12.1; metrics via \texttt{piq} (FSIM, MS-SSIM) and \texttt{lpips}; statistics via SciPy \cite{virtanen_scipy_2020}. Full specifications and wall times are in Table~\ref{tab:methods:compute}.
