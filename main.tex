...
\begin{figure}[htbp]
  ...
  \caption{%
    \textbf{Uncertainty-guided clinical hotspot detection (Arm~S, NLL curriculum FT).}
    Each row shows one test case with four panels:
    (a)~noisy input,
    (b)~denoised reconstruction $\hat{\mu}$ from Arm~S,
    (c)~reconstruction residual $|\text{noisy} - \hat{\mu}|$ smoothed with a Gaussian kernel ($\sigma{=}3$\,px), used as a spatially informative aleatoric uncertainty proxy (inferno colourmap; brighter $=$ higher uncertainty),
    (d)~clinical hotspot overlay combining (b) with anatomical zone boundaries (faint grid; RUL/RML/RLL~$=$~blue tones, LUL/LLL~$=$~red tones, Med~$=$~green) and thresholded high-uncertainty regions (top-20\% of proxy pixels after morphological closing), coloured by severity: HIGH~$=$~red fill/box, MOD~$=$~orange fill/box.
    Each hotspot ($\text{H}1$, $\text{H}2$, \ldots$) is labelled with its attributed lobe abbreviation, severity tier, and morphological class (Nodular / Vascular-Linear / Irregular).
    \emph{Top row (Normal case):}
    High-uncertainty regions concentrate diffusely along vascular margins and the pleural interface --- characteristic of signal-dependent quantum noise at vessel boundaries.
    \emph{Bottom row (Pneumonia case):}
    Hotspots concentrate in the lower lobes and mediastinal border, consistent with the consolidation-driven local contrast gradients that produce elevated reconstruction residuals.
    The residual proxy is equivalent to what a perfectly calibrated heteroscedastic $\hat{\sigma}_a$ approximates, and directly grounds the spatial uncertainty structure in the model's actual reconstruction behaviour.
  }
