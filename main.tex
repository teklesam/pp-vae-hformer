...
\begin{table}[htbp]
\centering
\caption[Final training metrics per ablation arm]{Best validation PSNR and SSIM achieved during training for all ablation arms, together with the epoch at which the best checkpoint was saved (used for all test-set evaluations). $\ddagger$~=~preliminary result; training continuing beyond the reported epoch.}
...
B — Hformer + NLL                           & \multicolumn{2}{c}{(recovered from ep.\,190 ckpt)} & 190 \\
C — Hformer + NLL + SSIM                    & 32.137 & 0.8711 & 199 \\
D — Hformer + NLL + SSIM + FFL             & 33.264 & 0.8945 & 190 \\
E — PP-VAE-Hformer (full)                   & $31.432^\ddagger$ & $0.8562^\ddagger$ & $50^\ddagger$ \\
DnCNN \cite{zhang_beyond_2017}             & $32.487^\ddagger$ & $0.8756^\ddagger$ & $120^\ddagger$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:results:recon:mid} reports reconstruction performance on the 624-image held-out test set at mid noise level ($\eta = 200$, $n_{\mathrm{Normal}}=234$, $n_{\mathrm{Pneumonia}}=390$). Four metrics are reported: PSNR (pixel fidelity), SSIM (structural similarity), MS-SSIM (multi-scale structural similarity; Wang et al.\ 2003), and LPIPS (learned perceptual image patch similarity via AlexNet; Zhang et al.\ 2018; lower is better). Mean and SD are computed over all test images; 95\% CIs are from 1,000 bootstrap resamples. Arms~E and DnCNN results are preliminary ($\ddagger$; training continuing beyond epoch 50 and 120 respectively). All arms except E are deterministic; Arm~E uses a stochastic VAE bottleneck with $\mathbf{z} = \boldsymbol{\mu}_z$ at inference (deterministic decode).
...
\caption[Primary reconstruction results at mid noise level]{
  ...
  $\ddagger$ = preliminary (training in progress).}
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & ---        & $\mathbf{34.628 \pm 1.079}$ {[34.54, 34.72]} & $\mathbf{0.9137 \pm 0.0139}$ {[0.913, 0.915]} & $\mathbf{0.9731}$ & $\mathbf{0.1289}$ \\
B — NLL                            & ---        & $33.454 \pm 1.031$ {[33.37, 33.54]}          & $0.8947 \pm 0.0153$ {[0.894, 0.896]}          & $0.9645$          & $0.1597$ \\
C — NLL + SSIM                     & ---        & $32.358 \pm 1.018$ {[32.28, 32.44]}          & $0.8711 \pm 0.0174$ {[0.870, 0.872]}          & $0.9523$          & $0.1854$ \\
D — NLL + SSIM + FFL              & ---        & $33.431 \pm 1.018$ {[33.35, 33.51]}          & $0.8945 \pm 0.0152$ {[0.893, 0.896]}          & $0.9644$          & $0.1587$ \\
E — PP-VAE-Hformer                 & \checkmark & $31.628 \pm 0.950^\ddagger$ {[31.55, 31.70]} & $0.8568 \pm 0.0197^\ddagger$                  & $0.9471^\ddagger$ & $0.2031^\ddagger$ \\
\midrule
DnCNN \cite{zhang_beyond_2017}    & ---        & $32.694 \pm 0.952^\ddagger$ {[32.62, 32.77]} & $0.8756 \pm 0.0165^\ddagger$                  & $0.9544^\ddagger$ & $0.1526^\ddagger$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item PSNR and SSIM: higher is better. LPIPS: lower is better (AlexNet backbone, Zhang et al.\ \citeyear{zhang_unreasonable_2018}).
...
Three findings are noteworthy. First, Arms~B and~D are statistically indistinguishable ($p = 1.000$, $d = +0.022$), confirming that FFL precisely restores the PSNR penalty introduced by the SSIM term. Second, Arms~B and~D both outperform the DnCNN baseline by a large margin ($d \approx +0.76$, $p < 0.001$), demonstrating that NLL-based probabilistic training exceeds the MSE-trained deterministic baseline on pixel metrics while also producing calibrated uncertainty maps. Third, Arm~C (NLL + SSIM without FFL) performs \emph{worse} than DnCNN ($d = -0.341$, $p < 0.001$), showing that SSIM regularisation is harmful without the counterbalancing FFL high-frequency recovery. Arm~E results are preliminary ($\ddagger$, epoch 50/200) and are expected to improve with full training.
...
\caption[Bonferroni-corrected pairwise comparisons of PSNR]{
  ...
  Positive $d$ = Arm~1 $>$ Arm~2. *** $p<0.001$, n.s.\ not significant.
  $\ddagger$ = Arm~E/DnCNN preliminary (training in progress).}
\label{tab:results:stats:pairwise}
\begin{tabular}{llccc}
\toprule
\textbf{Arm 1} & \textbf{Arm 2} & \bm{$t$} & \bm{$p_{\mathrm{Bonf}}$} & \bm{$d$} \\
\midrule
A & B      & $+19.62$ & $9.99 \times 10^{-74}$  *** & $+1.111$ \\
A & C      & $+38.18$ & $1.07 \times 10^{-210}$ *** & $+2.162$ \\
A & D      & $+20.13$ & $4.52 \times 10^{-77}$  *** & $+1.140$ \\
A & E$^\ddagger$ & $+52.07$ & $< 10^{-300}$      *** & $+2.948$ \\
A & DnCNN  & $+33.53$ & $4.88 \times 10^{-175}$ *** & $+1.898$ \\
B & C      & $+18.88$ & $6.31 \times 10^{-69}$  *** & $+1.069$ \\
B & D      & $+0.39$  & $1.000$                 n.s. & $+0.022$ \\
B & E$^\ddagger$ & $+32.50$ & $4.20 \times 10^{-167}$ *** & $+1.840$ \\
B & DnCNN  & $+13.51$ & $9.58 \times 10^{-38}$  *** & $+0.765$ \\
C & D      & $-18.61$ & $3.09 \times 10^{-67}$  *** & $-1.054$ \\
C & E$^\ddagger$ & $+13.08$ & $1.47 \times 10^{-35}$ *** & $+0.741$ \\
C & DnCNN  & $-6.02$  & $3.44 \times 10^{-8}$   *** & $-0.341$ \\
D & E$^\ddagger$ & $+32.33$ & $8.45 \times 10^{-166}$ *** & $+1.830$ \\
D & DnCNN  & $+13.20$ & $3.61 \times 10^{-36}$  *** & $+0.747$ \\
E$^\ddagger$ & DnCNN & $-19.78$ & $9.03 \times 10^{-75}$ *** & $-1.120$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption[Full ablation results across all noise levels]{Full ablation results across all three noise presets (mean\,$\pm$\,SD). DnCNN is included as a fair external baseline (retrained on Kermany CXR with identical Poisson-Gaussian noise model). \textbf{Bold} = best per noise level. $\ddagger$ = preliminary result (training in progress).}
\label{tab:results:ablation:full}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{2}{c}{\textbf{Low noise}} & \multicolumn{2}{c}{\textbf{Mid noise}} & \multicolumn{2}{c}{\textbf{High noise}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
\textbf{Arm} & PSNR & SSIM & PSNR & SSIM & PSNR & SSIM \\
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$     & $\mathbf{36.587 \pm 1.081}$ & $\mathbf{0.9330}$ & $\mathbf{34.628 \pm 1.079}$ & $\mathbf{0.9137}$ & $\mathbf{32.911 \pm 1.067}$ & $\mathbf{0.8938}$ \\
B — NLL                               & $35.587 \pm 1.031$ & $0.9209$ & $33.454 \pm 1.031$ & $0.8947$ & $31.572 \pm 1.031$ & $0.8672$ \\
C — NLL + SSIM                        & $34.570 \pm 0.984$ & $0.9049$ & $32.358 \pm 1.018$ & $0.8711$ & $30.381 \pm 1.018$ & $0.8364$ \\
D — NLL + SSIM + FFL                 & $35.571 \pm 1.022$ & $0.9208$ & $33.431 \pm 1.018$ & $0.8945$ & $31.536 \pm 1.013$ & $0.8667$ \\
E — PP-VAE (full)$^\ddagger$          & $33.732 \pm 0.953$ & $0.8920$ & $31.628 \pm 0.950$ & $0.8568$ & $29.697 \pm 0.979$ & $0.8200$ \\
\midrule
DnCNN \cite{zhang_beyond_2017}$^\ddagger$ & $35.026 \pm 0.960$ & $0.9085$ & $32.694 \pm 0.952$ & $0.8756$ & $30.695 \pm 0.954$ & $0.8402$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:results:subgroup} stratifies reconstruction performance by diagnostic class. The Kermany test set comprises 234 Normal and 390 Pneumonia cases (ratio 38:62). Counterintuitively, \emph{all six arms} reconstruct Pneumonia cases significantly better than Normal cases by approximately 1~dB ($\Delta \in [-0.92, -1.12]$~dB, all $p < 0.001$). This finding is consistent across loss functions, architectures, and training stages, suggesting a dataset-level effect rather than a model-specific one. The most plausible explanation is that alveolar consolidation raises mean pixel intensity in Pneumonia images relative to clear lung fields, providing stronger gradient signal during training and resulting in lower per-pixel reconstruction error. Notably, the $\Delta$ magnitude is smallest for Arm~A ($-0.923$~dB) and largest for Arm~C ($-1.123$~dB), consistent with the SSIM penalty in Arm~C over-regularising lung-field texture. This finding has implications for AI-based triage: any model in this family will systematically exhibit lower uncertainty (and higher reconstruction quality) for pathological cases, which is a desirable property for clinical deployment but should be stated explicitly to avoid misinterpretation as general bias.
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & $34.051 \pm 0.936$ & $34.973 \pm 1.009$ & $-0.923$ dB & *** \\
B — NLL                            & $32.801 \pm 0.838$ & $33.846 \pm 0.933$ & $-1.045$ dB & *** \\
C — NLL + SSIM                     & $31.656 \pm 0.777$ & $32.779 \pm 0.907$ & $-1.123$ dB & *** \\
D — NLL + SSIM + FFL              & $32.797 \pm 0.832$ & $33.812 \pm 0.924$ & $-1.015$ dB & *** \\
E — PP-VAE$^\ddagger$              & $30.979 \pm 0.747$ & $32.018 \pm 0.839$ & $-1.039$ dB & *** \\
DnCNN \cite{zhang_beyond_2017}$^\ddagger$ & $32.057 \pm 0.761$ & $33.077 \pm 0.844$ & $-1.020$ dB & *** \\
\bottomrule
\end{tabular}
\end{table}
