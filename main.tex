...
\begin{table}[htbp]
\centering
\caption[Final training metrics per ablation arm]{Final validation PSNR and SSIM at epoch 200 for all ablation arms, together with the epoch at which the best validation PSNR was achieved (used for all test-set evaluations).}
\label{tab:results:training}
\begin{tabular}{lccc}
\toprule
\textbf{Arm} & \textbf{Best val PSNR (dB)} & \textbf{Best val SSIM} & \textbf{Best epoch} \\
\midrule
A — Hformer + $\mathcal{L}_{\mathrm{MSE}}$ & 34.623\textsuperscript{†} & 0.9269\textsuperscript{†} & [pending] \\
B — Hformer + NLL                           & 33.430 & 0.9090 & [pending] \\
C — Hformer + NLL + SSIM                    & 32.352 & 0.8867 & [pending] \\
D — Hformer + NLL + SSIM + FFL             & 33.437 & 0.9090 & [pending] \\
E — PP-VAE-Hformer (full)                   & \multicolumn{3}{c}{\textit{training in progress}} \\
DnCNN (Zhang et al., 2017)                 & \multicolumn{3}{c}{\textit{training in progress}} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:results:recon:mid} reports reconstruction performance on the 624-image held-out test set at mid noise level ($\eta = 200$, $n_{\mathrm{Normal}}=234$, $n_{\mathrm{Pneumonia}}=390$). Mean and standard deviation are computed over all test images; 95\% confidence intervals are derived by bootstrapping with 1,000 resamples. Arms~E and the DnCNN baseline are still training at the time of writing; their entries will be populated upon completion. Arm~A was evaluated in a prior single-pass run (denoted $\dagger$; bootstrap CI pending re-evaluation). All models with a VAE column marked `---' are deterministic; Arm~E uses a stochastic bottleneck with $\mathbf{z} = \mathbf{z}_\mu$ at inference.
...
\caption[Primary reconstruction results at mid noise level]{
  ...
  Values are mean\,$\pm$\,SD; square brackets give 95\% bootstrap confidence intervals.
  \textbf{Bold} = best per metric. \dag\ = the only model providing calibrated uncertainty.}
...
\midrule
A — Hformer + $\mathcal{L}_{\mathrm{MSE}}$  & ---         & $34.623^\dagger$                            & $0.9269^\dagger$ \\
B — Hformer + NLL                            & ---         & $33.430 \pm 1.028$ {[33.35, 33.51]}         & $0.9090 \pm 0.0164$ \\
C — Hformer + NLL + SSIM                     & ---         & $32.352 \pm 1.010$ {[32.27, 32.43]}         & $0.8867 \pm 0.0188$ \\
D — Hformer + NLL + SSIM + FFL              & ---         & $\mathbf{33.437 \pm 1.023}$ {[33.36, 33.52]}& $\mathbf{0.9090 \pm 0.0163}$ \\
E — PP-VAE-Hformer\dag                       & \checkmark  & \multicolumn{2}{c}{\textit{training in progress}} \\
\midrule
DnCNN (Zhang et al., 2017) \cite{zhang_beyond_2017} & ---  & \multicolumn{2}{c}{\textit{training in progress}} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Values populated from \texttt{metrics\_summary.csv} generated by \texttt{evaluate\_all.py}.
\item \dag\ Arm E is the only model that produces aleatoric and epistemic uncertainty maps (Section~\ref{sec:results:calibration}).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
A one-way ANOVA on PSNR at mid noise across Arms~B, C, and~D (Arms~A and~E pending) yielded $F(2,\,1869) = 233.60,\;p < 0.001$, confirming that at least one arm differs significantly from the others. Post-hoc pairwise comparisons with Bonferroni correction (3~comparisons; adjusted $\alpha = 0.017$) are reported in Table~\ref{tab:results:stats:pairwise}. Cohen's $d$ is the pooled effect-size estimator; values $>0.8$ indicate a large practical difference \cite{cohen_statistical_1988}. Notably, Arms~B and~D are statistically indistinguishable ($p = 1.000$, $d = -0.007$), indicating that the FFL component precisely offsets the PSNR cost introduced by the SSIM term.

\begin{table}[htbp]
\centering
\caption[Bonferroni-corrected pairwise comparisons of PSNR]{
  Pairwise $t$-test results (Bonferroni-corrected) for PSNR at mid noise level.
...
\midrule
A & B & \multicolumn{3}{c}{\textit{pending Arm A re-evaluation}} \\
A & C & \multicolumn{3}{c}{\textit{pending Arm A re-evaluation}} \\
A & D & \multicolumn{3}{c}{\textit{pending Arm A re-evaluation}} \\
A & E & \multicolumn{3}{c}{\textit{pending Arm E training}} \\
B & C & $+18.68$ & $2.36 \times 10^{-68}$ *** & $+1.057$ \\
B & D & $-0.12$  & $1.000$ n.s.              & $-0.007$ \\
B & E & \multicolumn{3}{c}{\textit{pending Arm E training}} \\
C & D & $-18.85$ & $1.87 \times 10^{-69}$ *** & $-1.067$ \\
C & E & \multicolumn{3}{c}{\textit{pending Arm E training}} \\
D & E & \multicolumn{3}{c}{\textit{pending Arm E training}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption[Full ablation results across all noise levels]{Full ablation results across all three noise presets (mean\,$\pm$\,SD). DnCNN is included as a fair external baseline (retrained on Kermany CXR with identical Poisson-Gaussian noise model).}
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$     & \textit{pending} & \textit{pending} & $34.623^\dagger$ & $0.9269^\dagger$ & \textit{pending} & \textit{pending} \\
B — NLL                               & \textit{pending} & \textit{pending} & $33.430 \pm 1.028$ & $0.9090 \pm 0.0164$ & \textit{pending} & \textit{pending} \\
C — NLL + SSIM                        & \textit{pending} & \textit{pending} & $32.352 \pm 1.010$ & $0.8867 \pm 0.0188$ & \textit{pending} & \textit{pending} \\
D — NLL + SSIM + FFL                 & \textit{pending} & \textit{pending} & $\mathbf{33.437 \pm 1.023}$ & $\mathbf{0.9090 \pm 0.0163}$ & \textit{pending} & \textit{pending} \\
E — PP-VAE (full)                     & \multicolumn{6}{c}{\textit{training in progress}} \\
\midrule
DnCNN \cite{zhang_beyond_2017}        & \multicolumn{6}{c}{\textit{training in progress}} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:results:subgroup} stratifies reconstruction performance by diagnostic class. The Kermany test set comprises 234 Normal and 390 Pneumonia cases (ratio 38:62). Counterintuitively, all three evaluated arms reconstruct Pneumonia cases significantly better than Normal cases ($\Delta \approx -1$~dB, all $p < 0.001$). This is likely attributable to the stronger pixel signal in consolidated lung regions: alveolar consolidation raises mean pixel intensity relative to clear lung fields, providing a richer gradient signal for the loss functions and reducing relative reconstruction error. This finding inverts the expectation that pathological features are harder to reconstruct and has implications for bias in AI-based triage systems. If the PP-VAE framework differentially preserves pathological features, we would expect Arm~E to maintain or widen this advantage relative to Arm~A. Full comparison including Arm~A, Arm~E, and DnCNN will be reported upon completion of training.
...
\midrule
A — $\mathcal{L}_{\mathrm{MSE}}$  & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} \\
B — NLL                            & $32.782 \pm 0.831$ & $33.819 \pm 0.934$ & $-1.038$ dB & *** \\
C — NLL + SSIM                     & $31.647 \pm 0.769$ & $32.775 \pm 0.895$ & $-1.127$ dB & *** \\
D — NLL + SSIM + FFL              & $32.803 \pm 0.834$ & $33.818 \pm 0.932$ & $-1.015$ dB & *** \\
E — PP-VAE                         & \multicolumn{4}{c}{\textit{training in progress}} \\
DnCNN \cite{zhang_beyond_2017}     & \multicolumn{4}{c}{\textit{training in progress}} \\
\bottomrule
\end{tabular}
\end{table}
