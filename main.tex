...
DnCNN \cite{zhang_beyond_2017} established residual learning for blind Gaussian denoising, achieving 29.23\,dB on BSD68 at $\sigma=25$. RED-CNN \cite{chen_low-dose_2017} adapted the U-Net architecture to low-dose CT, preserving fine structures via skip connections. DRUNet \cite{zhang_drunet_2021} (32.7M parameters, noise-conditioned) set the CNN ceiling at 29.48\,dB. All CNN denoisers share the fundamental limitation that MSE training drives regression to the conditional mean of all plausible clean images, producing spatial blurring that destroys the sharp margins of consolidating pneumonia \cite{zhao_loss_2017}.
...
MSE training drives networks to output the conditional mean of all plausible clean images, producing spatial blurring. Zhao et al.\ \cite{zhao_loss_2017} showed that replacing MSE with composite MS-SSIM+L1 loss improves perceptual quality by 0.5--1.0\,dB without architecture changes. The Charbonnier loss \cite{charbonnier_two_1994} adopted by SwinIR shares similar properties but still treats pixels as independent.
...
Table~\ref{tab:results:noise_stats} quantifies the three presets. The High preset (noisy-to-clean PSNR $\approx 13.0$~dB before any model inference) represents a clinically extreme dose reduction. Because the Poisson term $ay$ scales with signal, dark anatomical regions have worse SNR despite lower absolute noise, explaining why aleatoric uncertainty maps show elevated uncertainty at pleural angles and retrocardiac spaces. Evaluation was verified as deterministic by confirming identical noisy images for a 20-case subset across different random seed checks.
