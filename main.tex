...
Within this constraint, differences between arms are still informative. Arm~E (linear KL annealing) shows the least structure, with a near-spherical, overlapping cluster consistent with near-posterior-collapse (ARI~$=0.024$): the KL term has driven nearly all latent dimensions to the prior, leaving little variance to distinguish images. Arm~F (cyclical KL only) achieves the widest class separation of all five arms (ARI~$=0.310$): the periodic KL resets allow the encoder to temporarily encode diagnostically relevant information before the KL pressure resumes. Arm~G (free-bits KL, ARI~$=0.045$) shows only modest recovery from collapse, as the KL floor maintains continuous pressure on every channel. Arm~H (cyclical + free-bits, ARI~$=0.187$) achieves the second-best separation: wide latent dispersion is visible, but the free-bits floor partially suppresses the periodic KL release that gives Arm~F its advantage. Arm~P (PReLU full model, ARI~$=0.031$) shows near-collapse despite using the same cyclical+free-bits schedule as Arm~H, confirming that the PReLU decoder instability corrupts latent structure alongside reconstruction quality.

\begin{figure}[htbp]
\centering
\evalfig{figures/evaluation_v2/latent_space/latent_arm_e_ppvae}
\caption[Latent space projections -- Arm E (PP-VAE, linear KL)]{%
  t-SNE (left) and UMAP (right) of Arm~E latent codes ($n=450$, 150 per class).
  Points coloured by ground-truth label: Normal (blue), Bacterial (red), Viral (orange).
  ARI~$=0.024$, Silhouette~$=0.008$. The near-spherical, overlapping distribution is
...
\caption[Latent space projections -- Arm F (PP-VAE, cyclical KL)]{%
  Arm~F latent space (same layout). ARI~$=\mathbf{0.310}$, Silhouette~$=0.105$ ---
  ...
  Moderate cluster separation is visible between Normal (blue) and Pneumonia (red/orange),
...
\caption[Latent space projections -- Arm G (PP-VAE, free-bits KL)]{%
  Arm~G latent space. ARI~$=0.045$, Silhouette~$=0.036$. The free-bits mechanism
...
\caption[Latent space projections -- Arm H (PP-VAE, cyc+fb KL)]{%
  Arm~H latent space. ARI~$=0.187$, Silhouette~$=0.064$.
  ...
  Arm~F's superior class separation (ARI $0.187$ vs.\ $0.310$).}
...
\caption[Latent space projections -- Arm P (PP-VAE full model, PReLU)]{%
  Arm~P latent space. ARI~$=0.031$, Silhouette~$=0.048$.
  Near-collapse despite using the cyclical+free-bits schedule, confirming that
  PReLU decoder instability corrupts latent structure alongside reconstruction quality.}
...
\caption[Bacterial vs.\ Viral Pneumonia subgroup PSNR]{%
  ...
  attributable to the higher local contrast of lobar consolidation vs.\ diffuse interstitial pattern.}
\label{tab:results:subgroup:viral}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lccccc}
\toprule
\textbf{Arm} & \textbf{Normal (dB)} & \textbf{Bacterial (dB)} & \textbf{Viral (dB)} &
...
E — PP-VAE (linear KL)             & $30.982\pm0.762$ & $32.190\pm0.826$ & $31.740\pm0.790$ & $+1.208$ & $+0.758$ \\
