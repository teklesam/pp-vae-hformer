...
PP-VAE-Hformer & 24,943,618 & $\approx$9--10 & \textit{pending} & VAE bottleneck + dual head \\
\midrule
\multicolumn{5}{l}{\textit{Retrained KAIR baselines}} \\
DnCNN \cite{zhang_beyond_2017}  & 557,057    & $\approx$14 & \textit{pending} & 17-layer residual CNN \\
IRCNN \cite{zhang_learning_2017}& 185,857    & $\approx$14 & \textit{pending} & 5-layer modular PnP \\
FFDNet \cite{zhang_ffdnet_2018} & 486,980    & $\approx$14 & \textit{pending} & $\sigma$-conditioned CNN \\
DRUNet \cite{zhang_drunet_2021} & 32,651,329 & $\approx$14 & \textit{pending} & deep U-Net, $\sigma$-cond.\ \\
SwinIR \cite{liang_swinir_2021}$^\dagger$ & 11,497,681 & $\approx$35 & \textit{pending} & Swin Transformer \\
\midrule
\multicolumn{5}{l}{\textit{Extended SOTA baselines}} \\
NAFNet \cite{chen_nafnet_2022}  & 115,980,609 & $\approx$8  & \textit{pending} & simple gate, AMP enabled \\
SCUNet \cite{zhang_scunet_2023} & 57,043,457  & $\approx$45 & \textit{pending} & SC-hybrid, AMP disabled \\
\bottomrule
\multicolumn{5}{p{0.94\linewidth}}{\footnotesize
  Parameter counts verified from model state dicts loaded on NVIDIA A100 (CSD3).
  Inference latency (\textit{pending}) will be populated from benchmarking results
  (200 forward passes, 20 warmup, batch size 1, $256{\times}256$ input, A100 80\,GB).
