...
\textbf{Methods.}
A Foi Poisson-Gaussian degradation pipeline ($\alpha{=}0.03$, $\beta{=}0.005$) simulated clinically realistic quantum noise at three severity levels on 5,216 training images from the Kermany paediatric chest radiograph dataset. A 21-arm ablation study evaluated the independent and combined effects of eight loss terms — L2, L1, Charbonnier, heteroscedastic Negative Log-Likelihood (NLL), Multi-Scale SSIM, Focal Frequency Loss (FFL), Sobel edge supervision, and VGG perceptual loss — together with three KL divergence annealing schedules (linear, cyclical, free-bits) and two-stage curriculum fine-tuning. All 21 arms and five retrained baseline architectures (DnCNN, IRCNN, FFDNet, DRUNet, SwinIR) were evaluated on 624 held-out test images using PSNR, SSIM, FSIM, LPIPS, and calibration metrics.

\textbf{Results.}
The optimal deterministic arm (Arm~A, $\mathcal{L}_2$) achieved $34.618 \pm 1.071$~dB PSNR, exceeding all five retrained baselines and BM3D ($30.716$~dB). The best NLL arm (Arm~D, NLL+SSIM+FFL) achieved $33.435$~dB PSNR — $+0.64$~dB above DnCNN — while simultaneously producing calibrated aleatoric uncertainty maps at a cost of $1.2$~dB relative to the MSE baseline. Cyclical KL annealing prevented posterior collapse, closing a $1.10$~dB gap versus linear warmup and enabling VAE arms to reach statistical parity with CNN baselines (Arm~H vs IRCNN: $d{=}{-}0.101$, n.s.). Concurrent SSIM and Sobel edge supervision caused a $-1.747$~dB interference collapse (Arm~M), a novel empirical finding with direct implications for composite loss design. Two-stage curriculum fine-tuning achieved the best perceptual quality in the study (Arm~R: LPIPS~$0.1226$, FSIM~$0.9332$).
...
With the clinical motivation established and the technical landscape mapped, Chapter~\ref{chap:methods} specifies the experimental design — the noise model, architecture, and 21-arm ablation study — that puts these questions to a rigorous test.
...
With the experimental apparatus fully specified, Chapter~\ref{chap:results} presents the outcomes: reconstruction performance across all 21 arms and five baselines, latent space geometry, uncertainty calibration, and subgroup behaviour across Normal, Bacterial Pneumonia, and Viral Pneumonia cases.
...
\multicolumn{8}{p{0.97\linewidth}}{\footnotesize
  \textit{Curriculum arms:} Arms~Q, R, S are pairwise statistically indistinguishable ($p_{\mathrm{Bonf}} \geq 0.68$, $|d| \leq 0.07$); all three are equivalent to Arm~A in PSNR ($d < 0.08$, n.s.). Arm~R achieves the best LPIPS (0.1226) of any arm in the study.} \\
\endlastfoot
%
% ── Table body ────────────────────────────────────────────────────────────────
...
\begin{itemize}
  ...
  \item \texttt{scripts/}, Figure generation and post-processing scripts 
  (including \texttt{generate\_dissertation\_figures.py})
\end{itemize}
