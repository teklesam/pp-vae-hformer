...
\caption[Experimental pipeline overview]{Overview of the experimental pipeline. Clean paediatric radiographs (Kermany dataset) 
  ...
  by PP-VAE-Hformer. The model is trained across five ablation arms sharing an 
  encoder--decoder architecture but differing in loss functions and the VAE bottleneck 
  (Arm~E). Outputs include a denoised image $\hat{\mu}_y$ (all arms), an aleatoric 
  uncertainty map $\log\hat{\sigma}^2_a$ (Arms~B--E), and an epistemic uncertainty 
  map $\hat{\sigma}^2_e$ via Monte Carlo sampling (Arm~E only)}
