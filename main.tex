...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    (c)~aleatoric uncertainty map $\hat{\sigma}_a$ (magma colourmap, brighter $=$ higher uncertainty),
    (d)~clinical hotspot overlay combining (b) with anatomical zone boundaries (faint grid; RUL/RML/RLL~$=$~blue tones, LUL/LLL~$=$~red tones, Med~$=$~green) and thresholded high-uncertainty regions (top-20\% of $\hat{\sigma}_a$ pixels, morphologically closed) coloured by severity: HIGH~$=$~red box, MOD~$=$~orange box.
    Each hotspot ($\text{H}1$, $\text{H}2$, \ldots) is labelled with its attributed lobe abbreviation, severity tier, and mean aleatoric uncertainty $\bar{\sigma}_a$.
    \emph{Top row (Normal case):}
    Uncertainty concentrates along vascular margins and the pleural interface where quantum noise is highest; no focal parenchymal hotspot is flagged in either lower lobe.
    \emph{Bottom row (Pneumonia case):}
    A compact, nodular hotspot (eccentricity~$< 0.60$) localises to the right lower lobe (RLL), consistent with the consolidation focus; a secondary vascular/linear hotspot traces the adjacent fissure margin.
    The contrast between cases demonstrates that $\hat{\sigma}_a$ maps encode clinically interpretable spatial structure beyond global quality metrics.
  }
