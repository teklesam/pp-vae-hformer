...
def _make_act(name: str, channels: int) -> nn.Module:
    """Factory for activation functions. PReLU is per-channel learnable."""
    if name == "prelu":
...
class ResBlock(nn.Module):
    def __init__(self, channels: int, act: str = "gelu"):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(channels, channels, 3, 1, 1, bias=False),
            nn.GroupNorm(min(8, channels), channels),
            _make_act(act, channels),
            nn.Conv2d(channels, channels, 3, 1, 1, bias=False),
            nn.GroupNorm(min(8, channels), channels),
        )
        self.act = _make_act(act, channels)
...
class HybridBlock(nn.Module):
    ...
    def __init__(self, channels: int, num_heads: int = 4, window_size: int = 8, act: str = "gelu"):
        super().__init__()
        self.cnn = ResBlock(channels, act=act)
...
class PPVAEHformer(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_scales):
            self.enc_blocks.append(
                nn.Sequential(*[HybridBlock(ch, num_heads, window_size, act=act) for _ in range(num_blocks)])
            )
        ...
        self.bottleneck = nn.Sequential(
            *[HybridBlock(ch, num_heads, window_size, act=act) for _ in range(num_blocks)]
        )
        ...
        for skip_ch in reversed(skip_channels):
            ...
            self.dec_blocks.append(
                nn.Sequential(*[HybridBlock(out_ch, num_heads, window_size, act=act) for _ in range(num_blocks)])
            )
            ch = out_ch
    ...
    def forward(
        ...
        log_sig2a = self.aleat_head(feat).clamp(-10.0, 4.0)

        # DataParallel cannot gather None across replicas; return zero placeholders.
        # The loss uses self._use_kl (not a None-check) so these are never consumed.
        if z_mu is None:
            z_mu = mu_rec.new_zeros(mu_rec.shape[0], 1, 1, 1)
            z_logvar = mu_rec.new_zeros(mu_rec.shape[0], 1, 1, 1)
